...
interface PendingCallStore {
     ...
     * Возвращает true, если переход состоялся и текущий поток/источник может продолжать резолв.
     * Возвращает false, если звонка нет или его уже обрабатывает/обработал другой поток.
     */
    suspend fun tryMarkResolving(callRequestId: String): Boolean
}
