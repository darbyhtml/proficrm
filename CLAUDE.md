...
## Язык — STRICT RUSSIAN-ONLY

**Правило:** отвечать пользователю **только на русском языке**. Без исключений.

**Applies to:**

- Все ответы Дмитрию (текст response).
- Commit messages (prefix типа `Fix(Module):` + русское описание).
- PR descriptions.
- Comments в code (если добавляешь новые).
- Documentation updates.
- Compact summaries (тоже на русском).

**Исключения (допустимо на английском):**

- Имена файлов, путей, URL.
- Код (Python / JS / CSS) — не меняем на русский.
- Технические термины-аббревиатуры: JWT, API, CRUD, SSE, CORS, WSGI, SQL, HTTP, TOTP, DRF, ORM, CSP, CDN, DNS.
- Git commit type prefix: `Fix(Module):`, `Feat(Module):`, `Harden(Module):`, `Docs(Module):`.
- Имена Patterns в документации: "Pattern 1: Audit-before-implement" (но объяснение — на русском).

**НЕ заменять на русский** (пиши как есть):

- Названия компонентов: Celery, Django, Redis, PostgreSQL.
- Названия tools: GitHub, Docker, nginx, Playwright.
- Product names: Xiaomi, Android, iOS.

**НИКОГДА:**

- ❌ Не switch на английский в середине ответа «потому что так проще».
- ❌ Не писать summary на английском «для reviewer».
- ❌ Не писать compact summary на английском.
- ❌ Не использовать английские фразы там, где есть русский аналог:
  - «leverage» → «использовать».
  - «align» → «согласовать».
  - «ship» → «выкатить».
  - «rubber-stamp» → «штамповать», «одобрять без review».
  - «blocker» → «блокер» (русская транслитерация OK).
  - «trade-off» → «компромисс» или «trade-off» (заимствовано достаточно).
  - «best practice» → «лучшая практика».

**Проверь себя:** если в твоём ответе >15% слов на английском вне исключений — это drift. Переписывай.
