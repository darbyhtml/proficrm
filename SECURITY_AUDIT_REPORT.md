...
- ✅ SEC-005: Вынесена магическая строка "none" в константу RESPONSIBLE_FILTER_NONE
- ✅ SEC-009: Убраны примеры паролей из env.example
- ✅ PROD-001: Дополнен env.example всеми переменными окружения
- ✅ REL-002: Добавлена валидация типов в _apply_company_filters (try/except для int())
