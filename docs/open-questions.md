...
**Контекст (original)**: Public-readiness scan (Phase 5) выявил 12 возможно-real phone numbers
...
### Q11 [2026-04-21] W0.5a — pre-flight status — **RESOLVED (W0.5a DEFERRED)**

**Resolution 2026-04-21**: User выбрал Path E — defer W0.5a prod deploy until W9
UX volna. Pre-flight info ниже остаётся valid как **snapshot on date of
decision**, но actual deploy теперь W9.10 stage. Pre-flight будет refreshed
before W9 actual deploy.

ADR: `docs/decisions/2026-04-21-defer-prod-deploy-to-w9.md`.

---

**Контекст (retained for W9 reference).** Pre-W0.5a cleanup session (2026-04-21) включает sanity check
...
### Q14 [2026-04-21] Ruff tech debt baseline — 15 rules ignored — **Confirmed W1 scope**

**Scope 2026-04-21 (post Path E)**: W1 cleanup задача как originally planned.
Не связан с prod deploy (deferred к W9). 15 noisy rules + amocrm per-file-ignore
стабильны до W1 refactor delete'ит большую часть legacy code.
