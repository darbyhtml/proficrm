...
- **observe_only**:
  ...
  - В `audit.ActivityEvent` должны появляться записи с `entity_type=policy` при обращениях к API/phone API (и частично UI).
- **enforce**:
  - Запрещённые правилами ресурсы должны блокироваться (403/PermissionDenied на API/phone API).
  - Проверить, что расширение `require_admin()` через policy работает только в режиме enforce.
