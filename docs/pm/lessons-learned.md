...
## Lesson 6: Compact summary discipline

**Date:** ongoing (context compaction в длинных sessions).

**Что происходит:**

После 50-100K messages в session — automatic context compaction. Первые 80% conversation сжимаются в summary, последние 20% остаются raw. Summary сохраняется в transcript файле session-id.

**Impact на PM:**

- Future PM-session читает transcript summary для continuity.
- Если summary написан плохо — context не восстанавливается, work re-investigated.

**Правила:**

1. **Уважай прошлый summary.** Не re-investigate уже закрытые items (qa_manager, nkv identification, UX-1 timeline).
2. **Verify если >2 weeks passed** — facts могли измениться:
   - Prod HEAD (ссылка на tag).
   - Staging state (smoke test).
   - Open hotlist items.
3. **Update stale facts в memory** (не в transcript) — например MEMORY.md в `~/.claude/projects/.../memory/`.
4. **При конфликте** — trust observed state, не memory.
