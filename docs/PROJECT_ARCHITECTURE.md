...
│
├── count_types.py             # Вспомогательный скрипт: анализ типов в CSV (для импорта)
├── debug_types.py             # Вспомогательный скрипт: отладка кодировки CSV
├── debug_types2.py           # Вспомогательный скрипт: отладка CSV полей
├── scan_rows.py               # Вспомогательный скрипт: анализ строк в Excel (для импорта)
├── inspect_csv.py             # Вспомогательный скрипт: инспекция структуры CSV
├── inspect_export.py          # Вспомогательный скрипт: инспекция экспорта из AmoCRM (Excel → JSON схема)
└── cloudflared.exe            # Cloudflare Tunnel (для разработки/тестирования)
```
...
**Корневая директория:**
- `count_types.py` — анализ типов в CSV файле (для импорта данных)
- `debug_types.py` — отладка кодировки CSV (проблемы с кириллицей)
- `debug_types2.py` — отладка CSV полей (проверка структуры)
- `scan_rows.py` — анализ строк в Excel файле (для импорта из AmoCRM)
- `inspect_csv.py` — инспекция структуры CSV (поля, непустые значения)
- `inspect_export.py` — инспекция экспорта из AmoCRM (Excel → JSON схема для анализа)
- `cloudflared.exe` — Cloudflare Tunnel (для разработки/тестирования, туннель к localhost)

**Назначение:** Вспомогательные утилиты для анализа данных перед импортом, не являются частью основного приложения.
