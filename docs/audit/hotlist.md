...
## ✅ CLOSED 2026-04-24 14:30 UTC — merge `claude/recursing-elgamal-c31a17` → `main` (R1 restored)

**Severity**: MEDIUM (R1 policy violation: feature-branch 46 commits ahead of main).
**Created**: после W1 design series + W10.2-early WAL-G + docs updates накопились.
**Closed**: 2026-04-24 14:30 UTC (via Executor, ~7 min total).

### Результат closure

- **Merge commit**: `4be76236` на `main` (push в `origin/main`).
- **46 commits** integrated через `--no-ff` (feature-branch history preserved в graph).
- **CI run** `24893438165` **success** (ruff, black, bandit, gitleaks, pytest, coverage ≥50, pip-audit).
- **Deploy-staging run** `24893620961` **success** с `DEPLOY FULLY COMPLETED` marker.
- **Post-deploy smoke**: 6/6 зелёных (Liveness/Readiness/Health/Home/Login/Feature flags API).
- **Containers**: 7/7 healthy, 0 pending migrations.
- **Prod**: не затронут (tag `release-v0.0-prod-current` `be569ad4` не менялся, Path E active).

### Pre-check discovery — `core.fileMode` cross-platform drift

Pre-check showed 6 файлов M в staging area (mode drift после `core.fileMode=true` на Windows host, где FS не honors exe bit). Fix: `git config core.fileMode=false` на Windows worktree — Windows ignore, Linux servers продолжают honor (через staging `git pull` filesystem берёт modes из git index).

Lesson L24 documented (`core.fileMode` setting должен быть platform-specific, не copy-paste между OS).

### Follow-ups

- **Feature-branch `claude/recursing-elgamal-c31a17`** существует (PM worktree использует). Удалить через `git branch -d claude/recursing-elgamal-c31a17` + `git push origin --delete ...` после worktree cleanup.
- **Node.js 20 deprecation в GitHub Actions** — зафиксирован отдельным hotlist follow-up ниже.
- **Prod deploy следующий тег W9.10** (accumulated deploy W0-W8), не сейчас.

---

## 🟡 HOTLIST NEW (CI infra): Node.js 20 deprecation в GitHub Actions

**Severity**: LOW (deadline September 2026, force-switch с June 2026).
**Created**: 2026-04-24 (обнаружено в CI run `24893438165` post-merge annotations).
**Status**: OPEN — no urgency, plan-ahead item.

### Evidence

CI warnings в `.github/workflows/ci.yml`:

```
Node.js 16 actions are deprecated... eventually Node.js 20 actions will default to Node.js 24
actions/checkout@v4
...
Upgrade all GitHub Actions references to Node.js 24-compatible major versions когда они станут доступны. Monitor:

- `actions/checkout` — latest.
- `actions/setup-python` — latest.
- `gitleaks/gitleaks-action` — следить за major bump.

### Timing

- **June 2026**: GHA force-switch default from Node 20 → Node 24.
- **September 2026**: Node.js 20 полностью deprecated в GHA.

### Action

- **Not required сейчас** — CI работает корректно.
- Добавить в Q3 2026 roadmap как "GHA action versions refresh".

### References

- CI run with warnings: https://github.com/darbyhtml/proficrm/actions/runs/24893438165.
