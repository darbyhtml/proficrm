...
# 5.1. Перестроение поискового индекса компаний (FTS, PostgreSQL)
# Обязательно при первом деплое с новым поиском; при последующих деплоях сигналы обновляют индекс.
# При очень большом числе компаний можно закомментировать и выполнить вручную в спокойное время.
