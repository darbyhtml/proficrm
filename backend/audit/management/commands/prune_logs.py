...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        activity_days = options["activity_days"] or getattr(
            settings, "ACTIVITY_EVENT_RETENTION_DAYS", 180
        )
        errorlog_days = options["errorlog_days"] or getattr(settings, "ERRORLOG_RETENTION_DAYS", 90)
        notification_days = options["notification_days"] or getattr(
    ...
    def _prune_activity_events(self, now, days, dry_run):
        ...
        if dry_run:
            self.stdout.write(
                f"ActivityEvent: {count} записей к удалению (старше {days} дней) [dry-run]"
            )
        else:
            deleted, _ = qs.delete()
            self.stdout.write(
                self.style.SUCCESS(f"ActivityEvent: удалено {deleted} записей (старше {days} дней)")
            )
    ...
    def _prune_error_logs(self, now, days, dry_run):
        ...
        if dry_run:
            self.stdout.write(
                f"ErrorLog: {count} записей к удалению (старше {days} дней, resolved=True) [dry-run]"
            )
        else:
            deleted, _ = qs.delete()
            self.stdout.write(
                self.style.SUCCESS(
                    f"ErrorLog: удалено {deleted} записей (старше {days} дней, resolved=True)"
                )
            )
    ...
    def _prune_notifications(self, now, days, dry_run):
        ...
        if dry_run:
            self.stdout.write(
                f"Notification: {count} записей к удалению (старше {days} дней, is_read=True) [dry-run]"
            )
        else:
            deleted, _ = qs.delete()
            self.stdout.write(
                self.style.SUCCESS(
                    f"Notification: удалено {deleted} записей (старше {days} дней, is_read=True)"
                )
            )
