...
class ErrorLog(models.Model):
    ...
    level = models.CharField(
        "Уровень", max_length=16, choices=Level.choices, default=Level.ERROR, db_index=True
    )
    message = models.TextField("Сообщение", blank=True, default="")
    exception_type = models.CharField(
        "Тип исключения", max_length=255, blank=True, default="", db_index=True
    )
        ...
        if request.POST:
            ...
            for key, value in request.POST.items():
                if any(
                    ...
                    for sensitive in ["password", "token", "secret", "key", "csrf"]
                ):
