...
class Command(BaseCommand):
    help = (
        ...
        "2. Задачи, где создатель ≠ исполнитель, не изменяются (могли быть специально отредактированы).\n"
    ...
    def handle(self, *args, **options):
        ...
        total = self_tasks_count + null_tasks_count
        ...
        self.stdout.write(
            self.style.WARNING(
                ...
                f"\nПримечание: Задачи, где создатель ≠ исполнитель, не изменяются\n"
                f"  (могли быть специально отредактированы и переведены в другой статус)."
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                f"  - Установлено 'Новая': {updated_null}"
            )
        )
