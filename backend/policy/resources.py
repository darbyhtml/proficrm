...
RESOURCES: tuple[PolicyResource, ...] = (
    ...
    PolicyResource(
        "ui:companies:bulk_transfer", "action", "Компании: массовая передача", sensitive=True
    ),
    PolicyResource("ui:companies:cold_call:toggle", "action", "Холодный звонок: отметить"),
    PolicyResource(
        "ui:companies:cold_call:reset", "action", "Холодный звонок: откат", sensitive=True
    ),
    PolicyResource(
        "ui:companies:delete_request:create",
        "action",
        "Компании: запрос на удаление (создать)",
        sensitive=True,
    ),
    PolicyResource(
        "ui:companies:delete_request:cancel",
        "action",
        "Компании: запрос на удаление (отклонить)",
        sensitive=True,
    ),
    PolicyResource(
        "ui:companies:delete_request:approve",
        "action",
        "Компании: запрос на удаление (подтвердить)",
        sensitive=True,
    ),
    ...
    PolicyResource(
        "ui:tasks:bulk_reassign", "action", "Задачи: массовое переназначение", sensitive=True
    ),
    ...
    PolicyResource(
        "ui:mail:settings:update", "action", "Почта: настройки SMTP (изменить)", sensitive=True
    ),
    ...
    PolicyResource(
        "ui:mail:unsubscribes:list", "action", "Почта: отписки (список)", sensitive=True
    ),
    PolicyResource(
        "ui:mail:unsubscribes:delete", "action", "Почта: отписки (удалить)", sensitive=True
    ),
    PolicyResource(
        "ui:mail:unsubscribes:clear", "action", "Почта: отписки (очистить всё)", sensitive=True
    ),
    ...
    PolicyResource(
        "ui:mail:campaigns:delete", "action", "Почта: кампании (удалить)", sensitive=True
    ),
    ...
    PolicyResource(
        "ui:mail:campaigns:recipients:add", "action", "Почта: получатели (добавить)", sensitive=True
    ),
    PolicyResource(
        "ui:mail:campaigns:recipients:delete",
        "action",
        "Почта: получатели (удалить)",
        sensitive=True,
    ),
    PolicyResource(
        "ui:mail:campaigns:recipients:bulk_delete",
        "action",
        "Почта: получатели (массово удалить)",
        sensitive=True,
    ),
    PolicyResource(
        "ui:mail:campaigns:recipients:generate",
        "action",
        "Почта: получатели (сгенерировать)",
        sensitive=True,
    ),
    PolicyResource(
        "ui:mail:campaigns:recipients:reset_failed",
        "action",
        "Почта: получатели (сбросить ошибки)",
        sensitive=True,
    ),
    PolicyResource(
        "ui:mail:campaigns:recipients:reset_all",
        "action",
        "Почта: получатели (сбросить включая отправленные)",
        sensitive=True,
    ),
    PolicyResource(
        "ui:mail:campaigns:clear", "action", "Почта: кампания (очистить)", sensitive=True
    ),
    PolicyResource(
    ...
    PolicyResource(
        "ui:mail:campaigns:test_send", "action", "Почта: кампания (тест отправки)", sensitive=True
    ),
    ...
    PolicyResource(
        "ui:notifications:mark_all_read", "action", "Уведомления: отметить все прочитанными"
    ),
    PolicyResource(
    ...
    PolicyResource(
        ...
        "action",
        "Settings: режим просмотра (изменить)",
        sensitive=True,
    ),
    PolicyResource(
        "ui:settings:view_as:reset",
        "action",
        "Settings: режим просмотра (сбросить)",
        sensitive=True,
    ),
    # ---- W2.1.4.1: settings user-mgmt + branches + general (13 resources) ----
    PolicyResource("ui:settings:dashboard", "page", "Settings: рабочий стол админки"),
    PolicyResource("ui:settings:announcements", "page", "Settings: объявления системы"),
    PolicyResource("ui:settings:branches", "page", "Settings: список подразделений"),
    PolicyResource(
        "ui:settings:branches:create",
        "action",
        "Settings: создать подразделение",
        sensitive=True,
    ),
    PolicyResource(
        "ui:settings:branches:edit",
        "action",
        "Settings: редактировать подразделение",
        sensitive=True,
    ),
    PolicyResource("ui:settings:users", "page", "Settings: список пользователей"),
    PolicyResource(
        "ui:settings:users:create",
        "action",
        "Settings: создать пользователя",
        sensitive=True,
    ),
    PolicyResource(
        "ui:settings:users:edit",
        "action",
        "Settings: редактировать пользователя",
        sensitive=True,
    ),
    PolicyResource(
        "ui:settings:users:magic_link:generate",
        "action",
        "Settings: сгенерировать magic link пользователя",
        sensitive=True,
    ),
    PolicyResource(
        "ui:settings:users:force_logout",
        "action",
        "Settings: принудительный logout пользователя",
        sensitive=True,
    ),
    PolicyResource(
        "ui:settings:users:form",
        "action",
        "Settings: форма пользователя (AJAX)",
    ),
    PolicyResource(
        "ui:settings:users:update",
        "action",
        "Settings: обновить пользователя (AJAX)",
        sensitive=True,
    ),
    PolicyResource(
        "ui:settings:users:delete",
        "action",
    ...
    PolicyResource(
        "api:company_notes:create", "action", "API: заметки компаний (create)", sensitive=True
    ),
    PolicyResource(
        "api:company_notes:update", "action", "API: заметки компаний (update)", sensitive=True
    ),
    PolicyResource(
        "api:company_notes:delete", "action", "API: заметки компаний (delete)", sensitive=True
    ),
