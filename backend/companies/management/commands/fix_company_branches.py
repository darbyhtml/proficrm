...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        companies_qs = Company.objects.filter(responsible__isnull=False).select_related(
            "responsible", "branch", "responsible__branch"
        )
        ...
        if limit > 0:
            ...
            self.stdout.write(
                self.style.WARNING(f"Ограничение: проверяем только первые {limit} компаний")
            )
