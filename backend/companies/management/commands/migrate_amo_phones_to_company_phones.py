...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Это только автоматически созданные контакты при импорте из AMO для хранения дополнительных телефонов/email
        # Обычные контакты (с положительным amocrm_contact_id или NULL) НЕ затрагиваются
        #
        # ВАЖНО: Команда НЕ переносит:
        # - Основной телефон компании (company.phone) - он остается как есть
        # - Телефоны из обычных контактов (с положительным amocrm_contact_id) - они остаются в ContactPhone
        # - Телефоны из контактов без amocrm_contact_id - они остаются в ContactPhone
        stub_contacts = Contact.objects.filter(amocrm_contact_id__lt=0).select_related("company")

        # Дополнительная проверка: убеждаемся, что это действительно служебные контакты
        # (обычно они не имеют ФИО или имеют только служебные данные)
        # Это защита от случайного переноса телефонов из обычных контактов

        if limit > 0:
            stub_contacts = stub_contacts[:limit]
            self.stdout.write(
                self.style.WARNING(f"Ограничение: обрабатываем только первые {limit} контактов")
            )
        ...
        for contact in stub_contacts:
            ...
            if not phones.exists():
                if verbose:
                    self.stdout.write(
                        f"  Пропуск: контакт {contact.id} (компания {company.name}) - нет телефонов"
                    )
                skipped_count += 1
                continue
            ...
            max_order = (
                CompanyPhone.objects.filter(company=company).aggregate(m=Max("order")).get("m")
            )
            ...
            for phone in phones:
                ...
                if (company.phone or "").strip() == normalized:
                    if verbose:
                        self.stdout.write(
                            f"  Пропуск: телефон {v} уже является основным для компании {company.name}"
                        )
                    skipped_count += 1
                    continue
                ...
                if CompanyPhone.objects.filter(company=company, value=normalized).exists():
                    if verbose:
                        self.stdout.write(
                            f"  Пропуск: телефон {v} уже существует в CompanyPhone для компании {company.name}"
                        )
                    skipped_count += 1
                    continue
                ...
                if not dry_run:
                    try:
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"  Ошибка при создании CompanyPhone для компании {company.name}, телефон {v}: {e}"
                            )
                        )
                        error_count += 1
                else:
            ...
            if migrated_phones:
                ...
                if verbose:
                    self.stdout.write(
                        f"  ✓ Компания {company.name}: перенесено {len(migrated_phones)} телефонов: {', '.join(migrated_phones)}"
                    )
                elif not dry_run:
                    self.stdout.write(
                        f"  ✓ Компания {company.name}: перенесено {len(migrated_phones)} телефонов"
                    )
