...
def get_transfer_targets(user: User) -> QuerySet[User]:
    ...
    Для РОП и директора филиала ограничивает список получателей только пользователями своего филиала.
    """
    ...
    # РОП и директор филиала могут передавать только внутри своего филиала
...
def can_transfer_companies(user: User, company_ids: list) -> dict:
    ...
    if user.is_superuser or user.role in (User.Role.ADMIN, User.Role.GROUP_MANAGER):
        companies = Company.objects.filter(id__in=company_ids).select_related(
            "responsible", "branch"
        )
        return {"allowed": list(company_ids), "forbidden": []}
    ...
    for company in companies:
        if can_transfer_company(user, company):
            ...
            forbidden.append({"id": str(company.id), "name": company.name, "reason": reason})
