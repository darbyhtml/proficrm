...
class OnlineStatusTracker:
    ...
    def get_available_users(cls, branch_id: Optional[int] = None) -> Dict[int, str]:
        ...
        qs = User.objects.filter(is_active=True, agent_profile__isnull=False)
    ...
    def set_status(cls, user_id: int, status: str):
        ...
        dispatcher.dispatch(
            Events.AGENT_STATUS_CHANGED, timezone.now(), {"user_id": user_id, "status": status}
        )
    ...
    def get_online_user_ids(cls, branch_id: Optional[int] = None) -> Set[int]:
        ...
        return {
            user_id
            for user_id, status in available_users.items()
