...
def get_client_ip(request) -> str:
    """
    Получить IP клиента. Приоритет:
    1. CF-Connecting-IP (Cloudflare — один доверенный IP)
    2. X-Forwarded-For последний элемент (добавлен Nginx, ближайший прокси)
    3. REMOTE_ADDR
    """
    import ipaddress

    # Cloudflare всегда ставит реальный IP клиента
    ...
    # X-Forwarded-For: берём ПОСЛЕДНИЙ элемент (добавленный нашим Nginx)
