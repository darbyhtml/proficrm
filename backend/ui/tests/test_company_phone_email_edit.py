...
class CompanyPhoneEmailEditTests(TestCase):
    def setUp(self):
        ...
        self.owner = User.objects.create_user(
            username="owner2", password="pass12345", role=User.Role.MANAGER
        )
        self.other = User.objects.create_user(
            username="other2", password="pass12345", role=User.Role.MANAGER
        )
        self.company = Company.objects.create(
            name="C", responsible=self.owner, phone="+79990001122", email="main@example.com"
        )
    ...
    def test_update_main_phone_success_and_format(self):
        ...
        resp = self.client.post(
            url, data={"phone": "8 (999) 000-33-44"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_update_company_phone_reject_duplicate_with_main(self):
        ...
        resp = self.client.post(
            url, data={"phone": "+79990001122"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_update_main_email_invalid(self):
        ...
        resp = self.client.post(
            url, data={"email": "not-an-email"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
        self.assertEqual(resp.status_code, 400)
        self.assertFalse(resp.json()["success"])
    ...
    def test_update_company_email_reject_duplicate_with_main(self):
        ...
        resp = self.client.post(
            url, data={"email": "MAIN@example.com"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
        self.assertEqual(resp.status_code, 400)
        self.assertFalse(resp.json()["success"])
    ...
    def test_forbidden_for_not_owner(self):
        ...
        url = f"/companies/{self.company.id}/main-phone/update/"
        resp = self.client.post(
            url, data={"phone": "+79990009988"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
