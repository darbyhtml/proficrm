...
def cold_calls_report_day(request: HttpRequest) -> HttpResponse:
    ...
    qs_base = (
        CallRequest.objects.filter(
            created_by=user, created_at__gte=day_start, created_at__lt=day_end, note="UI click"
        )
        .exclude(status=CallRequest.Status.CANCELLED)
        .select_related("company", "contact")
    )
    qs = qs_base.filter(is_cold_call=True).order_by("created_at").distinct()
    ...
    manual_companies = (
        ...
        .select_related("primary_cold_marked_by")
        .only(
            "id",
            "name",
            "contact_name",
    ...
    manual_contacts = (
        Contact.objects.filter(
            company__responsible=user, cold_marked_at__gte=day_start, cold_marked_at__lt=day_end
        )
        .select_related("company", "cold_marked_by")
        .only(
            "id",
            "first_name",
            "last_name",
    ...
    new_companies_count = Company.objects.filter(
        created_by=user, created_at__gte=day_start, created_at__lt=day_end
    ).count()

    # 3. Количество новых контактов (в компаниях, где пользователь ответственный)
    new_contacts_count = Contact.objects.filter(
        company__responsible=user, created_at__gte=day_start, created_at__lt=day_end
    ).count()
    ...
    for call in qs.iterator(chunk_size=500):
        ...
        if call.contact_id and call.contact:
            ...
            contact_name = (
                (getattr(call.company, "contact_name", "") or "").strip() if call.company_id else ""
            )
    ...
    for phone_obj in manual_contact_phones:
        ...
        t = timezone.localtime(phone_obj.cold_marked_at).strftime("%H:%M")
        company_name = (
            phone_obj.contact.company.name
            if phone_obj.contact and phone_obj.contact.company
            else ""
        )
        contact_name = (
            f"{phone_obj.contact.first_name} {phone_obj.contact.last_name}".strip()
            if phone_obj.contact
            else ""
        )
    ...
    next_date = (
        ...
        if target_date < timezone.localdate(timezone.now())
        else None
    )

    return render(
        request,
        "ui/reports/cold_calls_day.html",
        {
            "day_label": day_label,
            "prev_date": prev_date,
            "next_date": next_date,
...
def cold_calls_report_month(request: HttpRequest) -> HttpResponse:
    ...
    for ms in candidates:
        ...
        exists = (
            CallRequest.objects.filter(
                created_by=user, created_at__date__gte=ms, created_at__date__lt=me, note="UI click"
            )
            .exclude(status=CallRequest.Status.CANCELLED)
    ...
    qs_base = (
        CallRequest.objects.filter(
            created_by=user,
            created_at__date__gte=selected,
    ...
    # 2. Количество новых компаний
    new_companies_count = Company.objects.filter(
        created_by=user, created_at__gte=month_start_aware, created_at__lt=month_end_aware
    ).count()

    # 3. Количество новых контактов (в компаниях, где пользователь ответственный)
    new_contacts_count = Contact.objects.filter(
        company__responsible=user, created_at__gte=month_start_aware, created_at__lt=month_end_aware
    ).count()
    ...
    total_cold = (
        qs_count
        + manual_companies.count()
        + manual_contacts.count()
        + manual_company_phones.count()
        + manual_contact_phones.count()
    )
    lines = [
    ...
    return render(
        request,
        "ui/reports/cold_calls_month.html",
        {
            "month_label": _month_label(selected),
...
def cold_calls_report_last_7_days(request: HttpRequest) -> JsonResponse:
    ...
    for i in range(7):
        ...
        qs_base = CallRequest.objects.filter(
            created_by=user, created_at__gte=day_start, created_at__lt=day_end, note="UI click"
        ).exclude(status=CallRequest.Status.CANCELLED)
        cnt_calls = qs_base.filter(is_cold_call=True).distinct().count()
        # Ручные отметки
        cnt_manual = (
            Company.objects.filter(
                responsible=user,
    ...
    return JsonResponse(
        {"ok": True, "range": "last_7_days", "period": period_label, "total": total, "days": days}
    )
