...
def is_ip_rate_limited(
    ip: str, key_prefix: str, max_requests: int, window_seconds: int = 60
) -> bool:
    """
...
def record_failed_login_attempt(
    username: str, ip: str, reason: str = "invalid_credentials"
) -> None:
    """
    ...
    try:
        ...
        logger.warning(
            f"Failed to log failed login attempt for {username_lower}: {e}", exc_info=True
        )
    ...
    if attempts >= MAX_LOGIN_ATTEMPTS:
        ...
        try:
            ...
            logger.warning(
                f"Failed to log account lockout for {username_lower}: {e}", exc_info=True
            )
