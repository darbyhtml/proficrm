...
MIN_PHONE_DIGITS = (
    10  # Минимум цифр для валидного телефона (для РФ номеров: 10 цифр без кода страны)
)
...
def map_phone_enum_code(
    enum_code: str | None, field_name: str = "", result: AmoMigrateResult | None = None
    ...
    if enum_code:
        ...
        if enum_code_upper in PHONE_ENUM_ALLOWLIST:
            ...
            return (
                ContactPhone.PhoneType.OTHER
            )  # Дефолт для неизвестных (не WORK, чтобы не портить аналитику)
...
def normalize_phone_enhanced(raw: str | None) -> PhoneResult:
    ...
    if len(final_digits) < MIN_PHONE_DIGITS or len(final_digits) > MAX_PHONE_DIGITS:
        ...
        return PhoneResult(
            digits=digits,
            reason=f"invalid_after_normalization_{len(final_digits)}_digits",
            ext=ext_value,
        )
    ...
    return PhoneResult(digits=digits, e164=e164, valid=True, ext=ext_value, reason=reason)
...
def parse_phone_value(raw: str | None) -> ParsedPhoneValue:
    ...
    if normalized.isValid and normalized.phone_e164 and is_valid_phone(normalized.phone_e164):
        ...
        return ParsedPhoneValue(
            phones=phones, extension=extension or normalized.ext, comment=comment
        )
    else:
...
def _format_call_note(note_type: str, params: Any) -> str:
    ...
    phone = _as_text(
        p.get("phone") or p.get("phone_number") or p.get("number") or p.get("to") or p.get("from")
    )
...
def _analyze_contact_completely(contact: dict[str, Any]) -> dict[str, Any]:
    ...
    for cf in result["custom_fields"]:
        ...
        # Должность
        is_position = (
            field_code == "POSITION" or "должность" in field_name or "позиция" in field_name
        )
        ...
        # Примечание
        is_note = any(k in field_name for k in ["примеч", "комментар", "коммент", "заметк"]) or any(
            k in field_code for k in ["NOTE", "COMMENT", "REMARK"]
        )
...
def _extract_company_fields(
    amo_company: dict[str, Any], field_meta: dict[int, dict[str, Any]]
) -> dict[str, Any]:
    """
    ...
    def first(fid: int | None) -> str:
        ...
        return (vals[0] if vals else "")[
            :500
        ]  # обрезаем до разумного максимума (для дальнейшей обрезки по полям)
    ...
    fid_phone_skynet = (
        SKYNET_PHONE_FIELD_ID if (field_meta and SKYNET_PHONE_FIELD_ID in field_meta) else None
    ) or _find_field_id(field_meta, name_contains=["скайнет", "список телефонов"])
    ...
    fid_email = _find_field_id(
        field_meta, codes=["email"], name_contains=["email", "e-mail", "почта"]
    )
    fid_web = _find_field_id(field_meta, codes=["web"], name_contains=["сайт", "web"])
    fid_director = _find_field_id(
        field_meta, name_contains=["руководитель", "директор", "генеральный"]
    )
    fid_activity = _find_field_id(
        field_meta, name_contains=["вид деятельности", "вид деят", "деятельност"]
    )
    fid_employees = _find_field_id(field_meta, name_contains=["численность", "сотрудник", "штат"])
    fid_worktime = _find_field_id(
        field_meta, name_contains=["рабочее время", "часы работы", "режим работы", "работа с"]
    )
    fid_tz = _find_field_id(field_meta, name_contains=["часовой пояс", "таймзона", "timezone"])
    fid_note = _find_field_id(
        field_meta, name_contains=["примеч", "комментар", "коммент", "заметк"]
    )
    ...
    if fid_phone_skynet and fid_phone_skynet != fid_phone:
        ...
        logger.debug(
            f"_extract_company_fields: Skynet phone field found but same as main phone field (ID: {fid_phone_skynet})"
        )
    else:
...
class AmoMigrateResult:
    ...
    notes_seen: int = 0
    notes_processed: int = (
        0  # дошли до create/update/skip; сумма: would_add+would_update+skipped_existing (dry) или created+updated+skipped_existing (real)
    )
    notes_created: int = 0
    notes_skipped_existing: int = 0
    notes_skipped_no_changes: int = (
        0  # существующая заметка, обновление не требуется (= skipped_existing)
    )
    notes_updated: int = 0
    ...
    position_rejected_as_phone: int = (
        0  # сколько должностей распознано как телефон (устаревшее, используем position_phone_detected)
    )
    name_cleaned_extension_moved_to_note: int = 0  # сколько раз "доб./ext" вынесено из имени
    name_instructions_moved_to_note: int = (
        0  # алиас к name_cleaned_extension_moved_to_note (инструкции из имени в note)
    )
    ...
    fields_skipped_to_prevent_blank_overwrite: int = (
        0  # сколько полей пропущено, чтобы не затереть непустые значения
    )
    skynet_phone_values_rejected: int = (
        0  # Skynet-поле: значения, не распознанные как телефон (произвольный текст)
    )
    skynet_phones_added: int = 0  # CompanyPhone: добавлено номеров из поля 309609 с comment=SKYNET
    company_phones_rejected_invalid: int = (
        0  # CompanyPhone: пропущено добавление из-за невалидного номера (не E.164)
    )

    # Детальные причины для dry-run логирования
    skip_reasons: list[dict[str, Any]] = (
        None  # список причин пропуска/изменений: [{"type": "skip_POSITION", "reason": "looks like phone", "value": "...", "contact_id": ...}, ...]
    )
    ...
    def get_dry_run_report(self) -> dict[str, Any]:
        ...
        is_dry_run = (
            self.companies_would_create > 0
            or self.companies_would_update > 0
            or self.contacts_would_create > 0
            or self.contacts_would_update > 0
            or self.notes_would_add > 0
            or self.notes_would_update > 0
            or self.tasks_would_create > 0
            or self.tasks_would_update > 0
        )
...
def fetch_matched_amo_company_ids(
    ...
    *,
    ...
    created_by_map = {
        int(c["id"]): int(c["created_by"]) for c in matched if c.get("id") and c.get("created_by")
    }
...
def fetch_tasks_for_companies(client: AmoClient, company_ids: list[int]) -> list[dict[str, Any]]:
    ...
    logger.info(
        f"fetch_tasks_for_companies: запрашиваем задачи для {len(company_ids)} компаний (batch_size={batch_size})"
    )

    for i in range(0, len(company_ids), batch_size):
        ids = company_ids[i : i + batch_size]
        logger.debug(
            f"fetch_tasks_for_companies: батч {i//batch_size + 1}, компаний: {len(ids)}, IDs: {ids[:5]}..."
        )
        ...
        try:
            ...
            logger.debug(
                f"fetch_tasks_for_companies: получено {len(tasks_batch)} задач для батча {i//batch_size + 1}"
            )
    ...
    logger.info(
        f"fetch_tasks_for_companies: всего получено {len(out)} задач для {len(company_ids)} компаний"
    )
    return out


def fetch_notes_for_companies_bulk(
    client: AmoClient, company_ids: list[int], *, batch_size: int = 200
) -> list[dict[str, Any]]:
    """
...
def fetch_notes_for_companies(client: AmoClient, company_ids: list[int]) -> list[dict[str, Any]]:
    ...
    if _notes_bulk_supported is None:
        ...
        logger.debug(
            "fetch_notes_for_companies: bulk notes недоступен, используем per-company endpoint"
        )
...
def _fetch_notes_per_company(client: AmoClient, company_ids: list[int]) -> list[dict[str, Any]]:
    ...
    out: list[dict[str, Any]] = []
    logger.info(
        f"_fetch_notes_per_company: получаем заметки для {len(company_ids)} компаний через per-company endpoint"
    )
    ...
    for cid in company_ids:
        try:
            ...
            if notes_list:
                logger.debug(
                    f"_fetch_notes_per_company: получено {len(notes_list)} заметок для компании {cid}"
                )
        except Exception as e:
            logger.warning(
    ...
    logger.info(
        f"_fetch_notes_per_company: всего получено {len(out)} заметок для {len(company_ids)} компаний"
    )
    return out
...
def fetch_contacts_for_companies(client: AmoClient, company_ids: list[int]) -> list[dict[str, Any]]:
    ...
    logger.info(
        f"fetch_contacts_for_companies: начинаем поиск контактов для {len(company_ids)} компаний: {company_ids[:5]}..."
    )
    ...
    for idx, company_id in enumerate(company_ids):
        try:
            ...
            if idx % 10 == 0 or idx == len(company_ids) - 1:
                logger.info(
                    f"fetch_contacts_for_companies: запрашиваем компанию {company_id} с with=contacts ({idx + 1}/{len(company_ids)})"
                )
            company_data = client.get(
                f"/api/v4/companies/{company_id}",
                params={
                    "with": "custom_fields,contacts"
                },  # Только custom_fields и contacts, БЕЗ notes
            )
            ...
            if isinstance(company_data, dict):
                ...
                if isinstance(contacts, list) and contacts:
                    ...
                    if idx % 10 == 0 or len(contacts) > 0:
                        logger.debug(
                            f"fetch_contacts_for_companies: компания {company_id}: найдено {len(contacts)} контактов через with=contacts"
                        )
                    ...
                    for contact in contacts:
                        if isinstance(contact, dict):
                            ...
                            for comp_ref in existing_companies:
                                if (
                                    isinstance(comp_ref, dict)
                                    and int(comp_ref.get("id") or 0) == company_id
                                ):
                            ...
                            if method1_contacts_count == 0:
                                ...
                                logger.info(
                                    f"fetch_contacts_for_companies: структура первого контакта (id={contact_id_debug}):"
                                )
                                logger.info(f"  - has _embedded: {'_embedded' in contact}")
                                logger.info(
                                    f"  - _embedded.companies: {contact.get('_embedded', {}).get('companies', [])}"
                                )
                ...
                logger.warning(
                    f"fetch_contacts_for_companies: компания {company_id}: неожиданный тип ответа: {type(company_data)}"
                )
        except Exception as e:
            logger.warning(
                f"fetch_contacts_for_companies: ошибка при получении компании {company_id} с контактами: {e}",
                exc_info=True,
            )
    ...
    logger.info(
        f"fetch_contacts_for_companies: способ 1 (with=contacts): найдено {method1_contacts_count} контактов из {len(company_ids)} компаний"
    )
    ...
    if out:
        logger.info(
            f"fetch_contacts_for_companies: проверяем, нужны ли полные данные для {len(out)} контактов..."
        )
        ...
        for contact in out:
            ...
            if not has_custom_fields:
                ...
                logger.debug(
                    f"fetch_contacts_for_companies: контакт {contact_id} не имеет custom_fields_values, нужны полные данные"
                )
            else:
                custom_fields = contact.get("custom_fields_values") or []
                custom_fields_count = len(custom_fields) if isinstance(custom_fields, list) else 0
                logger.debug(
                    f"fetch_contacts_for_companies: контакт {contact_id} уже имеет custom_fields_values ({custom_fields_count} полей)"
                )

        # Запрашиваем полные данные только для контактов без custom_fields_values
        if contact_ids:
            logger.info(
                f"fetch_contacts_for_companies: запрашиваем полные данные для {len(contact_ids)} контактов без custom_fields_values..."
            )
            ...
            for i in range(0, len(contact_ids), batch_size):
                ...
                try:
                    ...
                    logger.warning(
                        f"fetch_contacts_for_companies: ошибка при получении полных данных контактов (batch {i//batch_size + 1}): {e}",
                        exc_info=True,
                    )
                    continue
            ...
            logger.info(
                f"fetch_contacts_for_companies: получено полных данных для {len(full_contacts_map)} контактов из {len(contact_ids)} запрошенных"
            )
            updated_out = []
            for contact in out:
                if not isinstance(contact, dict):
            ...
            out = updated_out
            logger.info(
                f"fetch_contacts_for_companies: обновлено {len(out)} контактов с полными данными"
            )
        else:
            logger.info(
                "fetch_contacts_for_companies: все контакты уже имеют custom_fields_values, дополнительный запрос не нужен"
            )

    # Если через with=contacts ничего не нашли, пробуем способ 2: filter[company_id] для каждого ID
    if not out:
        logger.info(
            "fetch_contacts_for_companies: через with=contacts контакты не найдены, пробуем filter[company_id] для каждой компании..."
        )
        ...
        for idx, company_id in enumerate(company_ids):
            try:
                ...
                if idx % 10 == 0:
                    logger.info(
                        f"fetch_contacts_for_companies: запрашиваем контакты через filter[company_id]={company_id} ({idx + 1}/{len(company_ids)})"
                    )
                ...
                if isinstance(contacts_data, dict):
                    ...
                    if isinstance(contacts, list) and contacts:
                        ...
                        logger.debug(
                            f"fetch_contacts_for_companies: компания {company_id}: найдено {len(contacts)} контактов через filter[company_id]"
                        )
                        ...
                        logger.info(
                            f"fetch_contacts_for_companies: компания {company_id}: контакты не найдены через filter[company_id] (пустой список)"
                        )
                else:
                    logger.warning(
                        f"fetch_contacts_for_companies: компания {company_id}: неожиданный тип ответа через filter[company_id]: {type(contacts_data)}"
                    )
            except Exception as e:
                logger.warning(
                    f"fetch_contacts_for_companies: ошибка при получении контактов через filter[company_id]={company_id}: {e}",
                    exc_info=True,
                )
                continue

        logger.info(
            f"fetch_contacts_for_companies: способ 2 (filter[company_id]): найдено {method2_contacts_count} контактов из {len(company_ids)} компаний"
        )

    logger.info(
        f"fetch_contacts_for_companies: ИТОГО найдено {len(out)} контактов из {len(company_ids)} компаний"
    )
    return out


def fetch_contacts_per_company_precise(
    client: AmoClient, company_ids: list[int]
    ...
    logger.info(
        f"fetch_contacts_per_company_precise: начинаем точное получение контактов для {len(company_ids)} компаний"
    )
    ...
    for idx, company_id in enumerate(company_ids):
        try:
            ...
            if contacts:
                logger.info(
                    f"fetch_contacts_per_company_precise: компания {company_id}: найдено {len(contacts)} контактов"
                )
                # Добавляем company_id к каждому контакту
                for contact in contacts:
                    if isinstance(contact, dict):
                        ...
                        if contact_id:
                            ...
                            if "_embedded" not in contact:
                                contact["_embedded"] = {}
                            if (
                                "companies" not in contact["_embedded"]
                                or not contact["_embedded"]["companies"]
                            ):
                                contact["_embedded"]["companies"] = [{"id": company_id}]
                ...
                logger.debug(
                    f"fetch_contacts_per_company_precise: компания {company_id}: контакты не найдены"
                )

        except Exception as e:
            logger.warning(
                f"fetch_contacts_per_company_precise: ошибка при получении контактов для компании {company_id}: {e}",
                exc_info=True,
            )
            continue

    logger.info(
        f"fetch_contacts_per_company_precise: ИТОГО найдено {len(all_contacts)} контактов для {len(company_ids)} компаний"
    )
    return all_contacts, contact_id_to_company_map


def fetch_contacts_medium_batch(
    client: AmoClient, company_ids: list[int]
) -> tuple[list[dict[str, Any]], dict[int, int]]:
    """
    ...
    logger.info(
        f"fetch_contacts_medium_batch: начинаем оптимизированное получение контактов для {len(company_ids)} компаний"
    )
    ...
    for idx, company_id in enumerate(company_ids):
        try:
            ...
            logger.warning(
                f"fetch_contacts_medium_batch: ошибка при получении компании {company_id}: {e}",
                exc_info=True,
            )
            continue
    ...
    if not all_contact_ids:
        logger.info(
            f"fetch_contacts_medium_batch: контакты не найдены для {len(company_ids)} компаний"
        )
        return [], {}

    logger.info(
        f"fetch_contacts_medium_batch: найдено {len(all_contact_ids)} уникальных контактов, запрашиваем полные данные..."
    )
    ...
    for i in range(0, len(contact_ids_list), batch_size):
        ...
        try:
            ...
            logger.warning(
                f"fetch_contacts_medium_batch: ошибка при получении полных данных контактов (batch {i//batch_size + 1}): {e}",
                exc_info=True,
            )
            continue

    logger.info(
        f"fetch_contacts_medium_batch: ИТОГО получено {len(all_contacts)} контактов с полными данными для {len(company_ids)} компаний"
    )
    return all_contacts, contact_id_to_company_map


def fetch_contacts_via_links(
    client: AmoClient, company_ids: list[int]
    ...
    logger.info(
        f"fetch_contacts_via_links: начинаем получение контактов для {len(company_ids)} компаний через Entity Links API"
    )
    ...
    for i in range(0, len(company_ids), batch_size):
        ...
        logger.info(
            f"fetch_contacts_via_links: запрашиваем связи для батча компаний {i//batch_size + 1} ({len(batch_company_ids)} компаний)"
        )
                ...
                if isinstance(links, list):
                    ...
                    if links:
                        try:
                            ...
                            logger.info(
                                f"fetch_contacts_via_links: LINKS SAMPLE={links_json_limited}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"fetch_contacts_via_links: ошибка при логировании links: {e}"
                            )
                    ...
                    for link in links:
                        ...
                        company_id = int(link.get("entity_id") or 0) or int(
                            link.get("from_entity_id") or 0
                        )
                    ...
                    logger.info(
                        f"fetch_contacts_via_links: обработано связей: processed={links_processed}, skipped_no_company={links_skipped_no_company}, skipped_no_contact={links_skipped_no_contact}, skipped_wrong_type={links_skipped_wrong_type}"
                    )
                    if skipped_types:
                        logger.info(
                            f"fetch_contacts_via_links: пропущенные типы связей: {dict(skipped_types)}"
                        )
                else:
                    logger.warning(
                        f"fetch_contacts_via_links: неожиданный тип links в ответе: {type(links)}"
                    )
            else:
                logger.warning(
                    f"fetch_contacts_via_links: неожиданный тип ответа: {type(links_data)}"
                )

        except Exception as e:
            logger.error(
                f"fetch_contacts_via_links: ошибка при получении связей для батча компаний: {e}",
                exc_info=True,
            )
    ...
    logger.info(
        f"fetch_contacts_via_links: найдено {len(all_contact_ids)} уникальных контактов для {len(company_ids)} компаний"
    )
    ...
    logger.info(
        f"fetch_contacts_via_links: запрашиваем полные данные для {len(contact_ids_list)} контактов (разбито на {(len(contact_ids_list) + contact_batch_size - 1) // contact_batch_size} батчей)"
    )

    for i in range(0, len(contact_ids_list), contact_batch_size):
        batch_contact_ids = contact_ids_list[i : i + contact_batch_size]
        logger.info(
            f"fetch_contacts_via_links: запрашиваем полные данные для батча контактов {i//contact_batch_size + 1} ({len(batch_contact_ids)} контактов)"
        )
        ...
        try:
            ...
            if isinstance(contacts_data, dict):
                ...
                if isinstance(contacts, list):
                    logger.info(
                        f"fetch_contacts_via_links: получено {len(contacts)} контактов для батча {i//contact_batch_size + 1}"
                    )
                    ...
                    logger.warning(
                        f"fetch_contacts_via_links: неожиданный тип contacts в ответе: {type(contacts)}"
                    )
            else:
                logger.warning(
                    f"fetch_contacts_via_links: неожиданный тип ответа для контактов: {type(contacts_data)}"
                )

        except Exception as e:
            logger.error(
                f"fetch_contacts_via_links: ошибка при получении полных данных контактов (batch {i//contact_batch_size + 1}): {e}",
                exc_info=True,
            )
            # НЕ делаем fallback - просто пропускаем этот батч
            continue
    ...
    logger.info(
        f"fetch_contacts_via_links: contacts fetched by ids={len(all_contacts)} из {len(contact_ids_list)} запрошенных"
    )

    logger.info(
        f"fetch_contacts_via_links: ИТОГО получено {len(all_contacts)} контактов с полными данными для {len(company_ids)} компаний"
    )
    if warnings:
...
def fetch_contacts_bulk(
    ...
    if len(company_ids) <= 10:
        logger.info(
            f"fetch_contacts_bulk: используем точный запрос для {len(company_ids)} компаний (небольшой батч)"
        )
        return fetch_contacts_per_company_precise(client, company_ids)
    elif len(company_ids) <= 30:
        logger.info(
            f"fetch_contacts_bulk: используем оптимизированный запрос для {len(company_ids)} компаний (средний батч)"
        )
        ...
        logger.info(
            f"fetch_contacts_bulk: разбиваем {len(company_ids)} компаний на подбатчи по 10 (оптимизированный подход)"
        )
        all_contacts: list[dict[str, Any]] = []
        ...
        for i in range(0, len(company_ids), batch_size):
            batch_company_ids = company_ids[i : i + batch_size]
            logger.info(
                f"fetch_contacts_bulk: обрабатываем подбатч {i//batch_size + 1} ({len(batch_company_ids)} компаний)"
            )

            batch_contacts, batch_map = fetch_contacts_per_company_precise(
                client, batch_company_ids
            )
            all_contacts.extend(batch_contacts)
            all_contact_id_to_company_map.update(batch_map)

        logger.info(
            f"fetch_contacts_bulk: ИТОГО получено {len(all_contacts)} контактов для {len(company_ids)} компаний (разбито на {len(company_ids)//batch_size + 1} подбатчей)"
        )
    ...
    logger.info(
        f"fetch_contacts_bulk: начинаем bulk-получение контактов для {len(company_ids)} компаний"
    )
    ...
    try:
        ...
        for i in range(0, len(company_ids), batch_size):
            ...
            logger.info(
                f"fetch_contacts_bulk: запрашиваем контакты для батча компаний {i//batch_size + 1} ({len(batch_company_ids)} компаний)"
            )
            ...
            def should_stop_pagination(current_contacts: list[dict]) -> bool:
                ...
                if len(current_contacts) >= 100 and len(found_company_ids_during_pagination) == len(
                    batch_company_ids_set
                ):
                    logger.info(
                        f"fetch_contacts_bulk: прерываем пагинацию - найдены контакты для всех {len(batch_company_ids_set)} компаний (получено {len(current_contacts)} контактов)"
                    )
                    return True
                ...
                if len(current_contacts) >= 2000:
                    found_percentage = (
                        len(found_company_ids_during_pagination) / len(batch_company_ids_set)
                        if batch_company_ids_set
                        else 0
                    )
                    if found_percentage >= 0.8:  # 80% компаний
                        logger.info(
                            f"fetch_contacts_bulk: прерываем пагинацию - получено {len(current_contacts)} контактов, найдено для {len(found_company_ids_during_pagination)}/{len(batch_company_ids_set)} компаний ({found_percentage*100:.1f}%)"
                        )
                        return True
                ...
                if len(current_contacts) >= 5000:
                    relevance_percentage = (
                        relevant_contacts_count / len(current_contacts) if current_contacts else 0
                    )
                    ...
                    if (
                        relevant_contacts_count > 0 and relevance_percentage < 0.1
                    ):  # Нашли хотя бы 1 И меньше 10% релевантных
                        logger.warning(
                            f"fetch_contacts_bulk: прерываем пагинацию - получено {len(current_contacts)} контактов, найдено {relevant_contacts_count} релевантных ({relevance_percentage*100:.1f}%). API возвращает все контакты, дальше получать не нужно."
                        )
                        return True
                        ...
                        logger.debug(
                            f"fetch_contacts_bulk: получено {len(current_contacts)} контактов, но релевантных не найдено. Продолжаем получать контакты..."
                        )
            ...
            if contacts_batch:
                logger.info(
                    f"fetch_contacts_bulk: получено {len(contacts_batch)} контактов для батча компаний (найдено контактов для {len(found_company_ids_during_pagination)}/{len(batch_company_ids)} компаний)"
                )
                all_contacts.extend(contacts_batch)
    except Exception as e:
        logger.warning(
            f"fetch_contacts_bulk: ошибка при bulk-запросе контактов: {e}, пробуем альтернативный способ"
        )
        ...
        try:
            ...
            logger.info(
                f"fetch_contacts_bulk: получено {len(all_contacts)} контактов (без фильтра), фильтруем локально"
            )
    ...
    logger.info(
        f"_fetch_contacts_bulk_old_unused: отфильтровано {len(filtered_contacts)} контактов из {len(all_contacts)} полученных для {len(company_ids)} компаний"
    )
    return filtered_contacts, contact_id_to_company_map


def fetch_notes_for_contacts(
    client: AmoClient, contact_ids: list[int]
...
def fetch_notes_for_contacts_bulk(
    client: AmoClient, contact_ids: list[int], *, batch_size: int = 200
) -> dict[int, list[dict[str, Any]]]:
    """
    ...
    logger.info(
        f"fetch_notes_for_contacts_bulk: начинаем bulk-получение заметок для {len(contact_ids)} контактов"
    )
    ...
    for i in range(0, len(contact_ids), batch_size):
        ...
        logger.info(
            f"fetch_notes_for_contacts_bulk: обрабатываем батч {batch_num}/{total_batches} ({len(batch_ids)} контактов)"
        )
        ...
        try:
            ...
            logger.info(
                f"fetch_notes_for_contacts_bulk: получено {len(notes)} заметок для батча {batch_num}"
            )
            ...
            if "404" in error_str or "Not Found" in error_str:
                logger.debug(
                    f"fetch_notes_for_contacts_bulk: заметки недоступны для батча {batch_num} (404 - это нормально для некоторых аккаунтов AmoCRM)"
                )
            else:
                logger.warning(
                    f"fetch_notes_for_contacts_bulk: ошибка при получении заметок для батча {batch_num}: {e}"
                )
            # Продолжаем для следующих батчей
            continue

    logger.info(
        f"fetch_notes_for_contacts_bulk: ИТОГО получено заметок для {len(out)} контактов из {len(contact_ids)} запрошенных"
    )
    return out
...
def _upsert_company_from_amo(
    *,
    ...
    name = (
        str(amo_company.get("name") or "").strip()[:255] or "(без названия)"
    )  # обрезаем name сразу
    ...
    if not dry_run:
        try:
            ...
            logger.error(
                f"Failed to save company in _upsert_company_from_amo (amo_id={amo_id}): {e}",
                exc_info=True,
            )
...
def migrate_filtered(
    *,
    ...
    target_responsible: (
        User | None
    ) = None,  # если задан — все компании назначаются этому пользователю в нашей CRM
) -> AmoMigrateResult:
    ...
    if target_responsible is not None:
        ...
        responsible_local = (
            _map_amo_user_to_local(amo_user_by_id.get(int(responsible_user_id)) or {})
            if amo_user_by_id
            else None
        )
    ...
    companies, pagination_meta = fetch_companies_by_responsible(
        client, responsible_user_id, with_contacts=False, return_meta=True
    )
    ...
    if skip_field_filter:
        ...
        for c in companies:
            if _custom_has_value(
                c, sphere_field_id, option_id=sphere_option_id, label=sphere_label
            ):
    ...
    def _run():
        ...
        for sub_batch_start in range(0, len(batch), SUB_TRANSACTION_SIZE):
            ...
            with transaction.atomic():
                for amo_c in sub_batch:
                    ...
                    if extra.get("legal_name"):
                        new_legal = str(extra["legal_name"]).strip()[
                            :255
                        ]  # сначала strip, потом обрезка до max_length=255
                        ...
                        if not old_legal:
                            ...
                            len(comp.legal_name) > 255
                        ):  # защита: если уже заполнено, но слишком длинное
                            comp.legal_name = comp.legal_name.strip()[:255]
                            changed = True
                            if dry_run:
                                company_updates_diff["legal_name"] = {
                                    "old": old_legal,
                                    "new": comp.legal_name,
                                }
                    if extra.get("inn"):
                    ...
                    if extra.get("kpp"):
                        new_kpp = str(extra["kpp"]).strip()[
                            :20
                        ]  # сначала strip, потом обрезка до max_length=20
                    ...
                    if extra.get("address"):
                        new_addr = str(extra["address"]).strip()[
                            :500
                        ]  # сначала strip, потом обрезка до max_length=500
                        ...
                        if not old_addr:
                            ...
                            len(comp.address) > 500
                        ):  # защита: если уже заполнено, но слишком длинное
                            comp.address = comp.address.strip()[:500]
                            changed = True
                            if dry_run:
                                company_updates_diff["address"] = {
                                    "old": old_addr,
                                    "new": comp.address,
                                }
                    ...
                    first_phone_comment = (
                        None  # комментарий/доб. из разбора первого телефона для comp.phone_comment
                    )
                    ...
                    if phones and not original_main_phone:
                        ...
                        if parsed.phones and is_valid_phone(parsed.phones[0]):
                            ...
                            if norm.isValid and norm.phone_e164 and is_valid_phone(norm.phone_e164):
                                ...
                                first_phone_comment = (
                                    parsed.comment or ("Комментарий к телефону: " + raw_first)
                                )[:255]
                        if parsed.phones and is_valid_phone(parsed.phones[0]):
                            comp.phone = new_phone
                            changed = True
                    ...
                    if not (comp.phone_comment or "").strip():
                        if (comp.phone or "").strip() or (phones and str(phones[0]).strip()):
                            ...
                            if parts:
                                ...
                                changed = True
                                if dry_run:
                                    company_updates_diff["phone_comment"] = {
                                        "old": "",
                                        "new": comp.phone_comment,
                                    }
                    ...
                    if extra.get("employees_count") and can_update("employees_count"):
                        try:
                            ec = int(
                                "".join(
                                    ch
                                    for ch in str(extra.get("employees_count") or "")
                                    if ch.isdigit()
                                )
                                or "0"
                            )
                            ...
                            if ec > MAX_EMPLOYEES_COUNT:
                                logger.warning(
                                    f"Company {comp.name}: employees_count {ec} exceeds maximum {MAX_EMPLOYEES_COUNT}, capping to maximum"
                                )
                            ...
                            if ec > 0 and comp.employees_count != ec:
                                ...
                                changed = True
                                if dry_run:
                                    company_updates_diff["employees_count"] = {
                                        "old": str(old_ec) if old_ec else "",
                                        "new": str(ec),
                                    }
                        except (ValueError, OverflowError) as e:
                            logger.warning(
                                f"Company {comp.name}: failed to parse employees_count '{extra.get('employees_count')}': {e}"
                            )
                            pass
                        ...
                        if m:
                            try:
                                ...
                                if (
                                    ...
                                    old_start = (
                                        str(comp.workday_start) if comp.workday_start else ""
                                    )
                                    old_end = str(comp.workday_end) if comp.workday_end else ""
                                    if can_update(
                                        "workday_start"
                                    ) and comp.workday_start != dt_time(h1, m1):
                                        comp.workday_start = dt_time(h1, m1)
                                        changed = True
                                        if dry_run:
                                            company_updates_diff["workday_start"] = {
                                                "old": old_start,
                                                "new": str(dt_time(h1, m1)),
                                            }
                                    if can_update("workday_end") and comp.workday_end != dt_time(
                                        h2, m2
                                    ):
                                        comp.workday_end = dt_time(h2, m2)
                                        changed = True
                                        if dry_run:
                                            company_updates_diff["workday_end"] = {
                                                "old": old_end,
                                                "new": str(dt_time(h2, m2)),
                                            }
                            except Exception:
                                pass
                        ...
                        prev.update(
                            {
                                ...
                                "workday_start": (
                                    str(comp.workday_start) if comp.workday_start else None
                                ),
                    ...
                    if changed and not dry_run:
                        try:
                            ...
                            logger.error(
                                f"Failed to save company {comp.name} (amo_id={comp.amocrm_company_id}): {e}",
                                exc_info=True,
                            )
                    ...
                    if len(norm_phone_parts) > 1 and not dry_run:
                        try:
                            ...
                            logger.error(
                                f"Failed to save phone for company {comp.name}: {e}", exc_info=True
                            )
                    if len(norm_email_parts) > 1 and not dry_run:
                        try:
                            ...
                            logger.error(
                                f"Failed to save email for company {comp.name}: {e}", exc_info=True
                            )
                    ...
                    if (extra_phones or skynet_phones) and not dry_run:
                        ...
                        existing_phones_raw = list(
                            CompanyPhone.objects.filter(company=comp).values_list(
                                "value", flat=True
                            )
                        )
                        ...
                        main_phone_normalized = (
                            _normalize_phone(comp.phone) if (comp.phone or "").strip() else ""
                        )
                        ...
                        for p in extra_phones:
                                ...
                                if ph_comment and ph_comment.strip():
                                    if not dry_run:
                                        merged_comment = merge_comment_segments(
                                            comp.phone_comment, ph_comment
                                        )
                                        if merged_comment != (comp.phone_comment or ""):
                                            comp.phone_comment = merged_comment
                                            comp.save(update_fields=["phone_comment"])
                                    logger.debug(
                                        f"Company {comp.name}: merged comment for main phone {normalized}: {ph_comment[:50]}"
                                    )
                                else:
                                    logger.debug(
                                        f"Company {comp.name}: skipping phone {v} (normalized: {normalized}) - same as main phone, no comment"
                                    )
                                continue
                            if normalized in existing_phones_normalized:
                                logger.debug(
                                    f"Company {comp.name}: skipping phone {v} (normalized: {normalized}) - duplicate"
                                )
                                continue
                            ...
                            existing_phones_normalized.add(
                                normalized
                            )  # Предотвращаем дубликаты в этом батче
                            next_order += 1
                        ...
                        for item in skynet_phones:
                            ...
                            if not is_valid_phone(
                                raw_value
                            ):  # защита: parse_skynet_phones уже отфильтровал
                                res.company_phones_rejected_invalid += 1
                                if res.company_phones_rejected_invalid <= 3:
                                    logger.debug(
                            ...
                            if main_phone_normalized and main_phone_normalized == norm:
                                ...
                                if skynet_comment and skynet_comment.strip():
                                    if not dry_run:
                                        merged_comment = merge_comment_segments(
                                            comp.phone_comment, skynet_comment
                                        )
                                        if merged_comment != (comp.phone_comment or ""):
                                            comp.phone_comment = merged_comment
                                            comp.save(update_fields=["phone_comment"])
                                    logger.debug(
                                        f"Company {comp.name}: merged Skynet comment for main phone {norm}: {skynet_comment[:50]}"
                                    )
                        ...
                        if phones_to_create:
                            try:
                                CompanyPhone.objects.bulk_create(
                                    phones_to_create, ignore_conflicts=True
                                )
                                suf = f" ({skynet_added} from Skynet)" if skynet_added else ""
                                logger.info(
                                    f"Company {comp.name}: bulk created {len(phones_to_create)} CompanyPhone records{suf}"
                                )
                            except Exception as e:
                                logger.error(
                                    f"Failed to bulk_create CompanyPhone for company {comp.name}: {e}",
                                    exc_info=True,
                                )
                    ...
                    if extra_emails and not dry_run:
                        ...
                        existing_emails = set(
                            CompanyEmail.objects.filter(company=comp).values_list(
                                "value", flat=True
                            )
                        )
                        ...
                        if emails_to_create:
                            try:
                                CompanyEmail.objects.bulk_create(
                                    emails_to_create, ignore_conflicts=True
                                )
                                logger.info(
                                    f"Company {comp.name}: bulk created {len(emails_to_create)} CompanyEmail records"
                                )
                            except Exception as e:
                                logger.error(
                                    f"Failed to bulk_create CompanyEmail for company {comp.name}: {e}",
                                    exc_info=True,
                                )
                    ...
                    _apply_spheres_from_custom(
                        amo_company=amo_c,
                        company=comp,
                        field_id=sphere_field_id,
                        dry_run=dry_run,
                        exclude_label="Новая CRM",
                    )
            ...
            if not dry_run:
                logger.info(
                    f"migrate_filtered: обработано {len(sub_batch)} компаний в под-транзакции ({sub_batch_start + 1}/{len(batch)})"
                )
        ...
        if import_tasks and amo_ids and not (dry_run and not import_tasks):
            logger.info(
                f"migrate_filtered: запрашиваем задачи для {len(amo_ids)} компаний (import_tasks={import_tasks}, dry_run={dry_run})"
            )
            tasks = fetch_tasks_for_companies(client, amo_ids)
            res.tasks_seen = len(tasks)
            logger.info(
                f"migrate_filtered: получено задач из API: {res.tasks_seen} для {len(amo_ids)} компаний"
            )
            ...
            if task_uids:
                for tsk in Task.objects.filter(
                    external_source="amo_api", external_uid__in=task_uids
                ):
                    if tsk.external_uid:
            ...
            logger.info(
                f"migrate_filtered: обрабатываем {len(tasks)} задач (до фильтрации по дедлайну/компании)"
            )
            for t in tasks:
                ...
                title = str(
                    t.get("text") or t.get("result") or t.get("name") or "Задача (amo)"
                ).strip()[:255]
                due_at = None
                ...
                if existing:
                    ...
                    if upd:
                        if dry_run:
                            ...
                            logger.debug(
                                f"DRY-RUN: would update task {tid} for company {company.id}"
                            )
                        else:
                            existing.save()
                            res.tasks_updated += 1
            ...
            if dry_run:
                logger.info(
                    f"migrate_filtered: задачи НЕ запрашиваются (dry_run={dry_run}, import_tasks={import_tasks})"
                )
            res.tasks_seen = 0
        ...
        if import_notes and amo_ids and not (dry_run and not import_notes):
            logger.info(
                f"migrate_filtered: запрашиваем заметки для {len(amo_ids)} компаний (import_notes={import_notes}, dry_run={dry_run})"
            )
            try:
                ...
                logger.info(
                    f"migrate_filtered: получено заметок из API: {res.notes_seen} для {len(amo_ids)} компаний (режим: {res.notes_fetch_mode})"
                )
            ...
            if dry_run:
                logger.info(
                    f"migrate_filtered: заметки НЕ запрашиваются (dry_run={dry_run}, import_notes={import_notes})"
                )
            res.notes_seen = 0
            notes = []
        ...
        if notes and not notes_error:
            ...
            if note_uids:
                for nn in CompanyNote.objects.filter(
                    external_source="amo_api", external_uid__in=note_uids
                ):
                    if nn.external_uid:
            ...
            for n in notes:
                        ...
                        if not dry_run:
                            ...
                            try:
                                if debug_count_for_extraction < 3:
                                    logger.debug(
                                        f"      -> Skipped note {nid} (no text, note_type={note_type})"
                                    )
                ...
                creator_id = int(
                    n.get("created_by")
                    or n.get("created_by_id")
                    or n.get("responsible_user_id")
                    or 0
                )
                if creator_id:
                ...
                try:
                    if created_ts:
                        ...
                        if ct > 10**12:
                            ct = int(ct / 1000)
                        created_label = timezone.datetime.fromtimestamp(
                            ct, tz=timezone.utc
                        ).strftime("%d.%m.%Y %H:%M")
                except Exception:
                    created_label = ""
                ...
                if note_type.lower().startswith("amomail"):
                    ...
                    import_meta = {
                        ...
                        "body": summ,
                        "attachments": (
                            int(attach_cnt) if attach_cnt not in (None, "", 0, "0") else 0
                        ),
                    ...
                    # SMS из amoCRM
                    sms_text = (
                        str(params.get("text") or "").strip() if isinstance(params, dict) else ""
                    )
                    sms_phone = (
                        str(params.get("phone") or "").strip() if isinstance(params, dict) else ""
                    )
                    lines = ["SMS"]
                    if sms_phone:
                ...
                if existing_note:
                    ...
                    if import_meta and existing_note.meta != import_meta:
                        ...
                        upd = True
                    if existing_note.author_id == actor.id and (
                        author is None or author.id != actor.id
                    ):
                    ...
                    if upd:
                        if dry_run:
                            ...
                            res.skipped_writes_dry_run += 1
                            logger.debug(
                                f"DRY-RUN: would update note {nid} for company {company.id}"
                            )
                        else:
                            existing_note.save()
                            res.notes_updated += 1
            ...
            logger.warning(
                f"migrate_filtered: этап обработки заметок пропущен из-за ошибки: {notes_error}"
            )
            if res.warnings is None:
                res.warnings = []
        ...
        if should_process_contacts:
            ...
            logger.info(
                f"migrate_filtered: ===== НАЧАЛО ИМПОРТА КОНТАКТОВ для {len(amo_ids)} компаний ====="
            )
            ...
            try:
                ...
                # Rate limiting применяется автоматически в AmoClient
                logger.info(
                    f"migrate_filtered: вызываем fetch_contacts_bulk для {len(amo_ids)} компаний..."
                )

                # fetch_contacts_bulk уже фильтрует контакты по компаниям и возвращает маппинг
                full_contacts, contact_id_to_company_map, contact_warnings = fetch_contacts_bulk(
                    client, amo_ids
                )
                res.contacts_seen = len(full_contacts)
                logger.info(
                    f"migrate_filtered: получено {res.contacts_seen} контактов из API для {len(amo_ids)} компаний (bulk-метод)"
                )
                ...
                if res.contacts_seen == 0:
                    logger.warning(
                        f"migrate_filtered: ⚠️ КОНТАКТЫ НЕ НАЙДЕНЫ для компаний {list(amo_ids)[:10]}. Отфильтровано: {res.contacts_seen}"
                    )
                ...
                if not dry_run and full_contacts:
                    ...
                    contact_ids_for_notes = [
                        int(c.get("id") or 0)
                        for c in full_contacts
                        if isinstance(c, dict) and c.get("id")
                    ]
                    if (
                        contact_ids_for_notes and len(contact_ids_for_notes) <= 50
                    ):  # Запрашиваем заметки только для небольших батчей
                        logger.info(
                            f"migrate_filtered: запрашиваем заметки для {len(contact_ids_for_notes)} контактов (bulk-метод)..."
                        )
                        try:
                            contact_notes_map = fetch_notes_for_contacts_bulk(
                                client, contact_ids_for_notes
                            )
                            logger.info(
                                f"migrate_filtered: получено заметок для {len(contact_notes_map)} контактов"
                            )
                        except RateLimitError as e:
                            ...
                            if res.warnings is None:
                                res.warnings = []
                            res.warnings.append(
                                f"Импорт заметок контактов прерван из-за rate limit (429): {e}"
                            )
                        except Exception as e:
                            # ОПТИМИЗАЦИЯ: не прерываем импорт при ошибке получения заметок (часто 404)
                            logger.warning(
                                f"migrate_filtered: ошибка при получении заметок контактов (пропускаем): {e}"
                            )
                            contact_notes_map = {}
                    elif contact_ids_for_notes:
                        logger.info(
                            f"migrate_filtered: пропускаем запрос заметок для {len(contact_ids_for_notes)} контактов (слишком много, ускоряет импорт)"
                        )
                ...
                if dry_run:
                    ...
                    logger.info(
                        f"migrate_filtered: создан словарь local_companies_by_amo_id для dry-run: {len(local_companies_by_amo_id)} компаний"
                    )
                else:
                ...
                logger.info(
                    f"migrate_filtered: ===== НАЧАЛО ОБРАБОТКИ {len(full_contacts)} КОНТАКТОВ ====="
                )
                    ...
                    if amo_contact_ids:
                        ...
                        existing_contacts_map = {
                            ...
                            for c in Contact.objects.filter(
                                amocrm_contact_id__in=amo_contact_ids
                        ...
                        if contact_ids_for_phones:
                            for phone in ContactPhone.objects.filter(
                                contact_id__in=contact_ids_for_phones
                        ...
                        logger.debug(
                            f"migrate_filtered: предзагружено {len(existing_contacts_map)} контактов, {len(existing_phones_map)} телефонов, {len(existing_emails_map)} почт"
                        )
                    else:
                        existing_contacts_map = {}
                        existing_phones_map = {}
                ...
                for ac_idx, ac in enumerate(full_contacts):
                    ...
                    if ac_idx < 5 or contacts_processed % 10 == 0:
                        logger.info(
                            f"migrate_filtered: обработка контакта {ac_idx + 1}/{len(full_contacts)} (processed: {contacts_processed}, skipped: {contacts_skipped}, errors: {contacts_errors})"
                        )
                    ...
                    try:
                        ...
                        if structure_logged_count < 3:
                            logger.debug(
                                f"===== RAW CONTACT STRUCTURE ({structure_logged_count + 1}) [index {ac_idx}] ====="
                            )
                        ...
                        if ac is None:
                            ...
                            logger.debug(
                                f"  - Has 'first_name': {'first_name' in ac}, value: {ac.get('first_name')}"
                            )
                            logger.debug(
                                f"  - Has 'last_name': {'last_name' in ac}, value: {ac.get('last_name')}"
                            )
                            logger.debug(
                                f"  - Has 'custom_fields_values': {'custom_fields_values' in ac}"
                            )
                            if "custom_fields_values" in ac:
                                cfv = ac.get("custom_fields_values")
                                logger.debug(
                                    f"  - custom_fields_values type: {type(cfv)}, length: {len(cfv) if isinstance(cfv, list) else 'not_list'}"
                                )
                                if isinstance(cfv, list) and len(cfv) > 0:
                                    logger.debug(f"  - First custom_field: {cfv[0]}")
                            logger.debug(
                                f"  - Has 'phone': {'phone' in ac}, value: {ac.get('phone')}"
                            )
                            logger.debug(
                                f"  - Has 'email': {'email' in ac}, value: {ac.get('email')}"
                            )
                            ...
                            try:
                                ...
                                if note_related_keys:
                                    logger.debug(
                                        f"  - ⚠️ Found note-related keys: {note_related_keys}"
                                    )
                        ...
                        if contact_id in contact_id_to_company_map:
                            ...
                            local_company = local_companies_by_amo_id.get(
                                amo_company_id_for_contact
                            )
                            # Если не нашли в словаре и это не dry-run, ищем в БД
                            if not local_company and not dry_run:
                                local_company = Company.objects.filter(
                                    amocrm_company_id=amo_company_id_for_contact
                                ).first()
                        ...
                        if not local_company:
                            ...
                            if debug_count < preview_limit_skip:
                                ...
                                last_name_skipped, first_name_skipped = _parse_fio(
                                    name_str, first_name_raw, last_name_raw
                                )
                                ...
                                debug_data = {
                                    ...
                                    "standard_fields": full_analysis_skipped.get(
                                        "standard_fields", {}
                                    ),
                                    "all_custom_fields": [
                                        {
                                    ...
                                    "custom_fields_count": len(
                                        full_analysis_skipped.get("custom_fields", [])
                                    ),
                                }
                        ...
                        all_extracted = [
                            e
                            for e in [name_extracted, first_name_extracted, last_name_extracted]
                            if e
                        ]
                        if all_extracted:
                        ...
                        last_name, first_name = _parse_fio(
                            name_cleaned, first_name_cleaned, last_name_cleaned
                        )

                        # ОТЛАДКА: логируем начало обработки контакта с улучшенным выводом
                        preview_count_before = (
                            len(res.contacts_preview) if res.contacts_preview else 0
                        )
                        if preview_count_before < 3:
                            logger.debug(
                                f"Processing contact {amo_contact_id} (parsed: last_name={last_name}, first_name={first_name})"
                            )
                            logger.debug(
                                f"  - raw: name={name_str}, first_name={first_name_raw}, last_name={last_name_raw}"
                            )
                            logger.debug(
                                f"  - local_company: {local_company.id if local_company else None}"
                            )
                            ...
                            if isinstance(ac, dict):
                                ...
                                logger.debug(
                                    f"  - id: {ac.get('id')}, name: {ac.get('name')}, first_name: {ac.get('first_name')}, last_name: {ac.get('last_name')}"
                                )
                                ...
                                if custom_fields_debug is None:
                                    ...
                                    logger.debug(
                                        f"  - custom_fields_values: list, length={len(custom_fields_debug)}"
                                    )
                                    ...
                                    for cf_idx, cf in enumerate(custom_fields_debug[:2]):
                                        if isinstance(cf, dict):
                                            ...
                                            if (
                                                ...
                                                if (
                                                    field_code == "PHONE"
                                                    or "телефон" in field_name.lower()
                                                ):
                                                    val_masked = _mask_phone(val)
                                                elif (
                                                    field_code == "EMAIL"
                                                    or "email" in field_name.lower()
                                                    or "почта" in field_name.lower()
                                                ):
                                                    val_masked = _mask_email(val)
                                                else:
                                                    val_masked = val[:50]  # Ограничиваем длину
                                                logger.debug(
                                                    f"    [{cf_idx}] field_id={cf.get('field_id')}, code='{field_code}', name='{field_name}', value='{val_masked}'"
                                                )
                                else:
                                    logger.debug(
                                        f"  - custom_fields_values: {type(custom_fields_debug)} (not list)"
                                    )
                            else:
                        ...
                        company_id_for_key = (
                            local_company.id
                            if local_company and hasattr(local_company, "id")
                            else None
                        )
                        existing_contact = existing_contacts_map.get(
                            (amo_contact_id, company_id_for_key)
                        )
                        ...
                        if (
                            ...
                            and dry_run
                            and local_company
                            and hasattr(local_company, "id")
                        ):
                            existing_contact = Contact.objects.filter(
                                amocrm_contact_id=amo_contact_id, company=local_company
                            ).first()
                        ...
                        if custom_fields_raw is None:
                            ...
                            logger.warning(
                                f"Contact {amo_contact_id}: custom_fields_values is not a list: {type(custom_fields_raw)}, ignoring"
                            )
                            custom_fields = []
                        ...
                        try:
                            if debug_count_for_extraction < 3:
                                logger.debug(
                                    f"Extracting data from custom_fields for contact {amo_contact_id}:"
                                )
                                logger.debug(
                                    f"  - custom_fields type: {type(custom_fields)}, length: {len(custom_fields)}"
                                )
                                ...
                                if isinstance(custom_fields, list) and len(custom_fields) > 0:
                                    logger.debug(
                                        f"  - ALL custom_fields ({len(custom_fields)} fields):"
                                    )
                                    for cf_idx, cf in enumerate(
                                        custom_fields[:5]
                                    ):  # Показываем первые 5
                                        if isinstance(cf, dict):
                                            field_name = str(cf.get("field_name") or "").strip()
                                            ...
                                            logger.debug(
                                                f"    [{cf_idx}] {field_name} ({field_code}): {first_val[:50]}"
                                            )
                                elif len(custom_fields) == 0:
                                    logger.debug(
                                        "  - ⚠️ custom_fields is empty list (no custom fields found)"
                                    )
                                else:
                                    logger.debug(
                                        f"  - ⚠️ custom_fields is not a list: {type(custom_fields)}"
                                    )
                        except (NameError, UnboundLocalError, Exception) as debug_err:
                            # Защита от ошибок в debug-логике - не валим миграцию
                            logger.debug(
                                f"Debug preview failed (non-critical): {debug_err}", exc_info=False
                            )
                            ...
                            if note_val_raw:
                                ...
                                if note_val and len(note_val) > 3 and not note_val.isdigit():
                                    if not note_text:
                                        ...
                                        if debug_count_for_extraction < 3:
                                            logger.debug(
                                                f"  -> ✅ Found note_text in direct field '{note_key}': {note_text[:100]}"
                                            )
                                    else:
                                        ...
                                        if debug_count_for_extraction < 3:
                                            logger.debug(
                                                f"  -> Appended note_text from direct field '{note_key}': {note_val[:100]}"
                                            )
                        ...
                        if isinstance(ac, dict) and "_embedded" in ac:
                            ...
                            if isinstance(embedded, dict) and "notes" in embedded:
                                ...
                                if isinstance(notes_list, list) and notes_list:
                                    if debug_count_for_extraction < 3:
                                        logger.debug(
                                            f"  -> Found {len(notes_list)} notes in _embedded.notes"
                                        )
                                    ...
                                    for note_idx, note_item in enumerate(notes_list):
                                        if isinstance(note_item, dict):
                                            ...
                                            note_val = (
                                                str(note_item.get("text") or "").strip()
                                                or str(note_item.get("note") or "").strip()
                                                or str(note_item.get("comment") or "").strip()
                                                or str(
                                                    note_item.get("note_type") or ""
                                                ).strip()  # иногда тип заметки содержит текст
                                            )
                                            ...
                                            if not note_val and "params" in note_item:
                                                ...
                                                if isinstance(params, dict):
                                                    note_val = (
                                                        ...
                                                        or str(params.get("comment") or "").strip()
                                                        or str(params.get("note") or "").strip()
                                                    )
                                            ...
                                            note_type_val = (
                                                str(note_item.get("note_type") or "")
                                                .strip()
                                                .lower()
                                            )
                                            is_service_note = note_type_val in [
                                                "call_out",
                                                "call_in",
                                                "call",
                                                "amomail",
                                            ...
                                            if note_val and len(note_val) > 5:
                                                ...
                                                if is_note_type:
                                                    ...
                                                    current_is_service = note_text and (
                                                        "call_" in str(note_text).lower()
                                                        or str(note_text).lower()
                                                        in [
                                                            "call_out",
                                                            "call_in",
                                                            "call",
                                                        ...
                                                        or len(str(note_text).strip()) < 10
                                                    )
                                                    if not note_text or current_is_service:
                                                        note_text = note_val[:255]
                                                        if debug_count_for_extraction < 3:
                                                            logger.debug(
                                                                f"  -> ✅ Found note_text in _embedded.notes[{note_idx}] (type={note_type_val}): {note_text[:100]}"
                                                            )
                                                    else:
                                                        combined = f"{note_text}; {note_val[:100]}"
                                                        note_text = combined[:255]
                                                        if debug_count_for_extraction < 3:
                                                            logger.debug(
                                                                f"  -> Appended note_text from _embedded.notes[{note_idx}] (type={note_type_val}): {note_val[:100]}"
                                                            )
                                                # Если это не служебная заметка и у нас еще нет примечания - берем её
                                                    ...
                                                    if debug_count_for_extraction < 3:
                                                        logger.debug(
                                                            f"  -> Found note_text in _embedded.notes[{note_idx}] (type={note_type_val}, not service): {note_text[:100]}"
                                                        )
                                                ...
                                                logger.debug(
                                                    f"  -> Skipped service note type '{note_type_val}' (not a real note)"
                                                )
                                    ...
                                    if parsed.phones:
                                        ...
                                        for phone_e164 in parsed.phones:
                                            if is_valid_phone(phone_e164):
                                                ...
                                                note_to_add = (
                                                    parsed.comment
                                                    or f"Комментарий к телефону: {phone_e164}"
                                                )
                                                if not note_text:
                                        ...
                                        note_to_add = (
                                            parsed.comment or f"Комментарий к телефону: {pv}"
                                        )
                                        if (
                                            parsed.rejected_reason
                                            and "instruction" in parsed.rejected_reason.lower()
                                        ):
                                ...
                                if isinstance(custom_fields, list) and len(custom_fields) > 0:
                                    ...
                                    for cf_idx, cf in enumerate(
                                        ...
                                        if isinstance(cf, dict):
                                            ...
                                            field_code = str(cf.get("field_code") or "").strip()
                                            logger.debug(
                                                f"    [{cf_idx}] id={cf.get('field_id')}, code='{field_code}', name='{field_name}'"
                                            )
                                elif len(custom_fields) == 0:
                                    logger.debug(
                                        "  - ⚠️ custom_fields is empty list (no custom fields found)"
                        ...
                        for cf_idx, cf in enumerate(custom_fields):
                            if not isinstance(cf, dict):
                                if debug_count_for_extraction < 3:
                                    logger.debug(
                                        f"  - [field {cf_idx}] Skipped: not a dict, type={type(cf)}"
                                    )
                                continue
                            field_id = int(cf.get("field_id") or 0)
                            # ВАЖНО: в amoCRM используется field_code (не code) и field_name (не name)
                            field_code = str(
                                cf.get("field_code") or ""
                            ).upper()  # PHONE, EMAIL в верхнем регистре
                            field_name = str(
                                cf.get("field_name") or ""
                            ).lower()  # "телефон", "должность"
                            field_type = str(
                                cf.get("field_type") or ""
                            ).lower()  # "multitext", "text", "date"
                            values = cf.get("values") or []
                            if not isinstance(values, list):
                                if debug_count_for_extraction < 3:
                                    logger.debug(
                                        f"  - [field {cf_idx}] Skipped: values not a list, type={type(values)}"
                                    )
                                continue

                            if debug_count_for_extraction < 3:
                                logger.debug(
                                    f"  - [field {cf_idx}] field_id={field_id}, field_code={field_code}, field_name={field_name}, field_type={field_type}, values_count={len(values)}"
                                )
                            ...
                            for v_idx, v in enumerate(values):
                                ...
                                is_email = (
                                    ...
                                    or "почта" in field_name
                                    or "e-mail" in field_name
                                )
                                # Должность: проверяем field_code="POSITION" или field_name содержит "должность"/"позиция"
                                is_position = (
                                    field_code == "POSITION"
                                    or "должность" in field_name
                                    or "позиция" in field_name
                                )
                                # Холодный звонок: проверяем field_id=448321 (из примера), field_name и field_type="date"
                                is_cold_call_date = (
                                    field_id
                                    == 448321  # Известный ID поля "Холодный звонок" из примера
                                    or (
                                        field_type == "date"
                                        and ("холодный" in field_name and "звонок" in field_name)
                                    )
                                )
                                # День рождения: проверяем field_type="birthday" или field_name содержит "день рождения"/"birthday"
                                is_birthday = (
                                    field_type == "birthday"
                                    or ("день" in field_name and "рождени" in field_name)
                                    or "birthday" in field_name.lower()
                                )
                                # Примечание/Комментарий (текстовое поле)
                                # Проверяем field_id=366537 (из примера), field_name, и field_code для большей надежности
                                is_note = (
                                    field_id == 366537  # Известный ID поля "Примечание" из примера
                                    or any(
                                        k in field_name
                                        for k in ["примеч", "комментар", "коммент", "заметк"]
                                    )
                                    or any(
                                        k in str(field_code or "").upper()
                                        for k in ["NOTE", "COMMENT", "REMARK"]
                                    )
                                )

                                if debug_count_for_extraction < 3:
                                    logger.debug(
                                        f"    [value {v_idx}] val={val[:50]}, is_phone={is_phone}, is_email={is_email}, is_position={is_position}, is_cold_call_date={is_cold_call_date}, is_birthday={is_birthday}, is_note={is_note}"
                                    )
                                ...
                                if is_phone:
                                    ...
                                    if original_enum_code:
                                        ...
                                        if enum_code_upper not in PHONE_ENUM_ALLOWLIST:
                                            ...
                                            if debug_count_for_extraction < 3:
                                                logger.debug(
                                                    f"      -> enum_code '{original_enum_code}' не в allowlist, замаплен в {ptype}"
                                                )
                                    ...
                                    if parsed.phones:
                                        ...
                                        for phone_e164 in parsed.phones:
                                            ...
                                            if is_valid_phone(phone_e164):
                                                ...
                                                phone_comment_parts = []
                                                if parsed.extension:
                                                    phone_comment_parts.append(
                                                        f"доб. {parsed.extension}"
                                                    )
                                                    res.phones_extracted_with_extension += 1
                                                if parsed.comment:
                                                ...
                                                phones.append(
                                                    (ptype, phone_e164, phone_comment[:255])
                                                )
                                                if debug_count_for_extraction < 3:
                                                    logger.debug(
                                                        f"      -> Added phone: {_mask_phone(phone_e164)} (type={ptype}, ext={parsed.extension}, comment={phone_comment[:50]})"
                                                    )
                                            else:
                                                # Телефон не валиден - переносим в note
                                                res.phones_rejected_invalid += 1
                                    ...
                                    if not parsed.phones:
                                        ...
                                        note_to_add = (
                                            parsed.comment or f"Комментарий к телефону: {val}"
                                        )
                                        ...
                                        res.skip_reasons.append(
                                            {
                                                "type": (
                                                    "skip_PHONE"
                                                    if parsed.rejected_reason
                                                    else "move_PHONE_text_to_NOTE"
                                                ),
                                                "reason": parsed.rejected_reason
                                                or "no_valid_phone_found",
                                                "value": (
                                                    val[:100]
                                                    if len(val) <= 100
                                                    else val[:50] + "..."
                                        ...
                                        if debug_count_for_extraction < 3:
                                            logger.debug(
                                                f"      -> Skipped non-phone value as note: '{val[:50]}' (reason={parsed.rejected_reason})"
                                            )
                                    ...
                                    if t in ("WORK",) or "раб" in field_name_lower:
                                        ...
                                        t in ("PRIV", "PERSONAL", "HOME")
                                        or "личн" in field_name_lower
                                        or "персон" in field_name_lower
                                    ):
                                    ...
                                    for ev in _split_multi(val):
                                        ...
                                        if ev_clean and validate_email(ev_clean):
                                            ...
                                            if debug_count_for_extraction < 3:
                                                logger.debug(
                                                    f"      -> Added email: {ev_clean} (type={etype})"
                                                )
                                            ...
                                            if debug_count_for_extraction < 3:
                                                logger.debug(
                                                    f"      -> Rejected invalid email format: {ev_clean[:50]}"
                                                )
                                elif is_position:
                                    ...
                                    if position_is_only_phone(val):
                                        ...
                                        res.position_rejected_as_phone += (
                                        ...
                                        res.skip_reasons.append(
                                            {
                                                ...
                                                "value": (
                                                    _mask_phone(val) if len(val) > 10 else val[:50]
                                                ),
                                                "contact_id": amo_contact_id,
                                                "field_name": field_name,
                                                "source": "POSITION",
                                        ...
                                        if (
                                            normalized.isValid
                                            and normalized.phone_e164
                                            and is_valid_phone(normalized.phone_e164)
                                        ):
                                            ...
                                            if debug_count_for_extraction < 3:
                                                logger.debug(
                                                    f"      -> Position '{val}' recognized as phone, extracted '{phone_value}', position kept as '{val[:50]}'"
                                                )
                                        else:
                                            ...
                                            if debug_count_for_extraction < 3:
                                                logger.debug(
                                                    f"      -> Position '{val}' not valid phone, keeping original position"
                                                )
                                    else:
                                    ...
                                    is_current_note_service = (
                                        not note_text
                                        or "call_" in str(note_text).lower()
                                        or note_text.lower()
                                        in ["call_out", "call_in", "call", "amomail", "sms", "task"]
                                        or len(str(note_text).strip())
                                        < 10  # Очень короткие значения тоже подозрительны
                                    )
                                    ...
                                    if is_current_note_service or not note_text:
                                        ...
                                        if debug_count_for_extraction < 3:
                                            logger.debug(
                                                f"      -> Found note_text in custom_field (field_name='{field_name}', field_code='{field_code}'): {note_text[:100]}"
                                            )
                                            if is_current_note_service:
                                                logger.debug(
                                                    f"      -> Replaced service note '{note_text[:50]}' with real note from custom_field"
                                                )
                                    else:
                                        ...
                                        if debug_count_for_extraction < 3:
                                            logger.debug(
                                                f"      -> Appended note_text from custom_field: {val[:100]}"
                                            )
                                    ...
                                    if cold_call_timestamp is None:
                                        try:
                                            ...
                                            if debug_count_for_extraction < 3:
                                                logger.debug(
                                                    f"      -> Found cold call date: {cold_call_timestamp} (from field_id={field_id})"
                                                )
                                        except (ValueError, TypeError):
                                            if debug_count_for_extraction < 3:
                                                logger.debug(
                                                    f"      -> Invalid cold call timestamp: {val}"
                                                )
                                            cold_call_timestamp = None
                                elif is_birthday:
                                    ...
                                    if birthday_timestamp is None:
                                        try:
                                            ...
                                            if debug_count_for_extraction < 3:
                                                logger.debug(
                                                    f"      -> Found birthday: {birthday_timestamp} (from field_id={field_id})"
                                                )
                                        except (ValueError, TypeError):
                                            if debug_count_for_extraction < 3:
                                                logger.debug(
                                                    f"      -> Invalid birthday timestamp: {val}"
                                                )
                                            birthday_timestamp = None
                                ...
                                res.skip_reasons.append(
                                    {
                                        ...
                                        "contact_id": amo_contact_id,
                                        "normalized": (
                                            phone_key if normalized_for_dedup.isValid else None
                                        ),
                                    }
                                )
                                continue
                        ...
                        if note_text and phones:
                            ...
                            if existing_comment:
                                ...
                                if debug_count_for_extraction < 3:
                                    logger.debug(
                                        f"  -> Merged note_text with existing comment in first phone: {combined_comment[:100]}"
                                    )
                            else:
                                ...
                                if debug_count_for_extraction < 3:
                                    logger.debug(
                                        f"  -> Applied note_text to first phone: {note_text[:100]}"
                                    )
                        elif debug_count_for_extraction < 3 and not note_text:
                            logger.debug(
                                f"  -> ⚠️ No note_text found for contact {amo_contact_id} (checked direct fields and custom_fields)"
                            )
                        ...
                        if isinstance(ac, dict):
                            ...
                            for note_key in [
                                ...
                                if note_key in ac:
                                    note_search_info.append(
                                        f"direct:{note_key}={bool(ac.get(note_key))}"
                                    )
                            # Проверяем _embedded
                            if "_embedded" in ac:
                                embedded = ac.get("_embedded") or {}
                                if isinstance(embedded, dict) and "notes" in embedded:
                                    ...
                                    if notes_count > 0:
                                        ...
                                        if note_types:
                                            note_search_info.append(
                                                f"note_types:{','.join(note_types)}"
                                            )
                                        # Показываем, есть ли текст в заметках
                                        has_text = False
                                        for note_item in notes_list[:3]:
                                            if isinstance(note_item, dict):
                                                if note_item.get("text") or note_item.get(
                                                    "params", {}
                                                ).get("text"):
                                                    has_text = True
                                                    break
                            ...
                            all_custom_fields_with_values = (
                                []
                            )  # Для отладки - показываем ВСЕ поля с их значениями
                            for cf in custom_fields:
                                if isinstance(cf, dict):
                                    field_id = cf.get(
                                        "field_id"
                                    )  # ВАЖНО: field_id может быть числом (366537)
                                    field_name = str(cf.get("field_name") or "").strip()
                                    ...
                                    all_custom_field_names.append(
                                        f"id={field_id} name={field_name} code={field_code}"
                                    )
                                    ...
                                    if values and isinstance(values, list) and len(values) > 0:
                                        ...
                                        if val_text:
                                            all_custom_fields_with_values.append(
                                                f"id={field_id} name={field_name} code={field_code} value={val_text[:50]}"
                                            )
                                    ...
                                    is_note_field = (
                                        ...
                                        or (
                                            field_id and str(field_id) in ["366537"]
                                        )  # Известные ID полей примечаний
                                    )

                                    if is_note_field:
                                        note_fields_in_custom.append(
                                            f"id={field_id} name={field_name}({field_code})"
                                        )
                                        ...
                                        if values and isinstance(values, list) and len(values) > 0:
                                            ...
                                            if val_text:
                                                ...
                                                note_search_info.append(
                                                    f"found_note_value:{val_text[:50]}"
                                                )
                                                if debug_count_for_extraction < 3:
                                                    logger.debug(
                                                        f"  -> ✅ Found note_text in custom_field id={field_id} name={field_name}: {note_text[:100]}"
                                                    )

                            # Добавляем информацию о всех полях для отладки
                            if all_custom_field_names:
                                note_search_info.append(
                                    f"all_fields:{','.join(all_custom_field_names)}"
                                )
                            if note_fields_in_custom:
                                note_search_info.append(
                                    f"note_fields:{','.join(note_fields_in_custom)}"
                                )
                            elif debug_count_for_extraction < 3:
                                # Если не нашли поля с примечаниями, логируем все поля
                                logger.debug(
                                    f"  -> ⚠️ No note fields found in custom_fields. All fields: {all_custom_field_names}"
                                )
                        ...
                        if cold_call_timestamp:
                            try:
                                ...
                                if debug_count_for_extraction < 3:
                                    logger.debug(
                                        f"      -> Cold call date: timestamp={cold_call_timestamp} -> {cold_marked_at_dt.isoformat()} (normalized to 00:00:00 UTC)"
                                    )
                            except Exception as e:
                                cold_marked_at_dt = None
                                if dry_run:
                        ...
                        debug_data = {
                            ...
                            "custom_fields_sample": (
                                custom_fields
                                if dry_run
                                else (custom_fields[:3] if custom_fields else [])
                            ),  # В dry-run показываем все поля
                        ...
                        logger.info(
                            f"migrate_filtered: обработка контакта {amo_contact_id}: debug_count={debug_count}, preview_limit={preview_limit}, local_company={'найдена' if local_company else 'не найдена'}"
                        )
                        if debug_count < preview_limit:
                            ...
                            contact_debug = {
                                ...
                                "extracted_cold_call": (
                                    cold_marked_at_dt.isoformat() if cold_marked_at_dt else None
                                ),
                                ...
                                "all_custom_fields": [
                                    {
                                        ...
                                        "is_used": (
                                            (
                                                str(cf.get("field_code") or "").upper()
                                                in ["PHONE", "EMAIL", "POSITION"]
                                            )
                                            or any(
                                                k in (str(cf.get("field_name") or "").lower())
                                                for k in [
                                                    "телефон",
                                                    "почта",
                                                    "email",
                                ...
                                "embedded_tags": full_analysis.get("embedded_data", {}).get(
                                    "tags", []
                                ),
                                "embedded_companies": full_analysis.get("embedded_data", {}).get(
                                    "companies", []
                                ),
                                "embedded_leads": full_analysis.get("embedded_data", {}).get(
                                    "leads", []
                                ),
                                "embedded_customers": full_analysis.get("embedded_data", {}).get(
                                    "customers", []
                                ),
                                "embedded_notes": full_analysis.get("embedded_data", {}).get(
                                    "notes", []
                                ),
                                "embedded_notes_count": len(
                                    full_analysis.get("embedded_data", {}).get("notes", [])
                                ),
                                # Метаинформация
                            ...
                            if preview_count < 3 and isinstance(ac, dict):
                                ...
                                try:
                                    ...
                                    contact_debug["full_structure"] = json.dumps(
                                        ac, ensure_ascii=False, indent=2
                                    )[:5000]
                                except Exception as e:
                                    contact_debug["full_structure"] = (
                                        f"JSON error: {e}\n{str(ac)[:2000]}"
                                    )
                            ...
                            res._debug_contacts_logged = debug_count + 1
                            logger.info(
                                f"migrate_filtered: ✅ контакт {amo_contact_id} добавлен в preview (всего в preview: {len(res.contacts_preview)})"
                            )

                            # ОТЛАДКА: логируем, что добавили в preview
                            if preview_count < 3:
                                logger.debug(
                                    f"Added contact {amo_contact_id} to preview (count: {debug_count + 1}):"
                                )
                                ...
                                logger.debug(
                                    f"  - custom_fields_count: {len(full_analysis.get('custom_fields', []))}"
                                )
                                logger.debug(
                                    f"  - all_custom_fields: {len(contact_debug.get('all_custom_fields', []))}"
                                )
                        else:
                            logger.info(
                                f"migrate_filtered: ⚠️ контакт {amo_contact_id} НЕ добавлен в preview (превышен лимит: {debug_count} >= {preview_limit})"
                            )
                            ...
                            if contacts_processed <= 3:
                                ...
                                logger.debug(
                                    f"  - custom_fields_values count: {len(custom_fields)}"
                                )
                                if custom_fields:
                                    logger.debug("  - custom_fields sample (first 3):")
                                    for idx, cf in enumerate(custom_fields[:3]):
                                        logger.debug(
                                            f"    [{idx}] field_id={cf.get('field_id')}, code={cf.get('code')}, name={cf.get('name')}, type={cf.get('type')}, values={cf.get('values')}"
                                        )
                                else:
                                    logger.debug("  - ⚠️ custom_fields_values пуст или отсутствует")
                                logger.debug(
                                    f"  - raw contact top-level keys: {list(ac.keys())[:15] if isinstance(ac, dict) else 'not_dict'}"
                                )
                                logger.debug(
                                    f"  - has phone field: {bool(ac.get('phone')) if isinstance(ac, dict) else False}"
                                )
                                logger.debug(
                                    f"  - has email field: {bool(ac.get('email')) if isinstance(ac, dict) else False}"
                                )

                    except Exception as e:
                        contacts_errors += 1
                        amo_contact_id_for_error = (
                            int(ac.get("id") or 0) if isinstance(ac, dict) else 0
                        )
                        import traceback

                        error_traceback = traceback.format_exc()
                        logger.error(
                            f"migrate_filtered: ❌ ОШИБКА при обработке контакта {ac_idx + 1}/{len(full_contacts)} (amo_id: {amo_contact_id_for_error}): {e}",
                            exc_info=True,
                        )
                        ...
                        if res.contacts_preview is None:
                            res.contacts_preview = []
                        if (
                            len(res.contacts_preview) < 100
                        ):  # Ограничиваем количество ошибок в preview
                    ...
                    if dry_run:
                        ...
                        planned_phones_skipped_invalid: list[dict[str, str]] = (
                            []
                        )  # Невалидные телефоны
                        ...
                        enum_mapped_info: dict[str, dict[str, str]] = (
                            {}
                        )  # Информация о маппинге enum_code
                        cold_call_date_info: dict[str, Any] = (
                            {}
                        )  # Информация о дате холодного звонка
                        ...
                        if existing_contact:
                            ...
                            old_is_cold_call = bool(
                                getattr(existing_contact, "is_cold_call", False)
                            )
                            try:
                                old_phones = [
                                    {
                            ...
                            try:
                                old_emails = [
                                    str(e.value or "") for e in existing_contact.emails.all()
                                ]
                            except Exception:
                                old_emails = []
                        ...
                        if existing_contact:
                            ...
                            if (
                                position
                                and _c_can_update_preview("position")
                                and (existing_contact.position or "") != position[:255]
                            ):
                                planned_field_changes["position"] = {
                                    "old": old_position,
                                    "new": position[:255],
                                }
                        else:
                        ...
                        old_phone_values = set(
                            [p.get("value") for p in (old_phones or []) if p.get("value")]
                        )
                        old_phone_values_normalized = (
                            set()
                        )  # Нормализованные номера для дедупликации
                        ...
                        for pt, pv, pc in phones:
                            ...
                            if (
                                phone_key in old_phone_values_normalized
                                or pv_db in old_phone_values
                            ):
                            ...
                            if dry_run:
                                planned_phones_add.append(
                                    {
                                        "value": (
                                            _mask_phone(pv_db) if len(pv_db) > 10 else pv_db[:50]
                                        ),
                                        "type": str(pt),
                        ...
                        old_email_values = set(
                            [str(e or "").strip().lower() for e in (old_emails or []) if e]
                        )
                        for et, ev in emails:
                        ...
                        if note_text and phones:
                            ...
                            if first_phone_val:
                                ...
                                if (
                                    existing_first
                                    and not (existing_first.get("comment") or "").strip()
                                ):
                                    planned_field_changes["first_phone_comment"] = {
                                        "old": "",
                                        "new": note_text[:255],
                                    }
                        ...
                        for cf in full_analysis.get("custom_fields", []):
                            ...
                            if field_code_upper == "PHONE" or "телефон" in field_name_lower:
                                ...
                                field_code_upper == "EMAIL"
                                or "email" in field_name_lower
                                or "почта" in field_name_lower
                            ):
                                is_used = True
                                usage_info.append("Email")
                            elif (
                                field_code_upper == "POSITION"
                                or "должность" in field_name_lower
                                or "позиция" in field_name_lower
                            ):
                                is_used = True
                                ...
                                field_type == "date"
                                and "холодный" in field_name_lower
                                and "звонок" in field_name_lower
                            ):
                                is_used = True
                        ...
                        if (
                            planned_field_changes
                            or planned_phones_add
                            or planned_emails_add
                            or all_custom_fields_info
                        ):
                            res.contacts_updates_preview.append(
                                {
                                    "company_name": local_company.name if local_company else "",
                                    "company_id": local_company.id if local_company else None,
                                    "contact_name": f"{last_name} {first_name}".strip()
                                    or "(без имени)",
                                    "amo_contact_id": amo_contact_id,
                    ...
                    if cold_call_timestamp:
                        try:
                            UTC = getattr(timezone, "UTC", dt_timezone.utc)
                            cold_marked_at_dt = timezone.datetime.fromtimestamp(
                                cold_call_timestamp, tz=UTC
                            )
                        except Exception:
                            cold_marked_at_dt = None
                    ...
                    if local_company:
                        cold_marked_by_user = (
                            local_company.responsible or local_company.created_by or actor
                        )
                    else:
                    ...
                    if existing_contact:
                        ...
                        old_note = (
                            contact.note or ""
                        )  # ВАЖНО: нормализуем None -> "" для корректного сравнения
                        ...
                        contact_changed = (
                            contact.first_name != old_first_name
                            or contact.last_name != old_last_name
                            or contact.position != old_position
                            or new_note != old_note
                            or contact.is_cold_call != old_is_cold_call
                            or contact.cold_marked_at != old_cold_marked_at
                            or birthday_timestamp is not None  # raw_fields всегда обновляем
                        )
                        ...
                        if dry_run:
                            if existing_contact:
                                ...
                                res.skipped_writes_dry_run += 1
                                logger.debug(
                                    f"DRY-RUN: would update contact {amo_contact_id} for company {local_company.id if local_company else None}"
                                )
                            else:
                                res.contacts_would_create += 1
                                res.skipped_writes_dry_run += 1
                                logger.debug(
                                    f"DRY-RUN: would create contact {amo_contact_id} for company {local_company.id if local_company else None}"
                                )
                        else:
                        ...
                        if dry_run:
                            ...
                            logger.debug(
                                f"DRY-RUN: would add {phones_added} phones, update {phones_updated} phones for contact {amo_contact_id}"
                            )
                        else:
                            for idx, (pt, pv, pc) in enumerate(phones):
                                ...
                                if obj is None:
                                    ...
                                    if obj.type != pt and (
                                        not obj.comment or obj.comment == phone_comment[:255]
                                    ):
                                        obj.type = pt
                                        upd = True
                                    if upd:
                            ...
                            if phones_to_update:
                                ContactPhone.objects.bulk_update(
                                    phones_to_update, ["type", "comment"]
                                )
                        ...
                        if dry_run:
                            ...
                            logger.debug(
                                f"DRY-RUN: would add {emails_added} emails for contact {amo_contact_id}"
                            )
                        else:
                            for et, ev in emails:
                                ...
                                if email_key not in existing_emails_map:
                                    emails_to_create.append(
                                        ContactEmail(contact=contact, type=et, value=ev_db)
                                    )
                                    emails_added += 1
                            ...
                            if emails_to_create:
                                try:
                                    ContactEmail.objects.bulk_create(
                                        emails_to_create, ignore_conflicts=True
                                    )
                                except Exception:
                            ...
                            if debug_count_after < 10:
                                logger.debug(
                                    f"  - Updated: phones={phones_added}, emails={emails_added}, position={bool(position)}"
                                )
                    else:
                        ...
                        if "amo" not in debug_data:
                            debug_data["amo"] = {}
                        debug_data["amo"]["custom_fields_values"] = (
                            ac.get("custom_fields_values") or []
                        )
                        ...
                        contact = Contact(
                            ...
                            note=(
                                note_text[:8000] if note_text else ""
                            ),  # Устанавливаем примечание при создании
                        ...
                        if dry_run:
                            ...
                            logger.debug(
                                f"DRY-RUN: would add {phones_count} phones, {emails_count} emails for new contact {amo_contact_id}"
                            )
                        else:
                            ...
                            if phones_to_create_new:
                                ContactPhone.objects.bulk_create(
                                    phones_to_create_new, ignore_conflicts=True
                                )
                            ...
                            if emails_to_create_new:
                                try:
                                    ContactEmail.objects.bulk_create(
                                        emails_to_create_new, ignore_conflicts=True
                                    )
                                except Exception:
                                    # Fallback: создаем по одному при ошибке
                            ...
                            if debug_count_after < 10:
                                logger.debug(
                                    f"  - Saved: phones={phones_added}, emails={emails_added}, position={bool(position)}"
                                )
                ...
                if not dry_run:
                    if contacts_to_create:
                        logger.info(
                            f"migrate_filtered: created {len(contacts_to_create)} new contacts"
                        )
                    if contacts_to_update:
                        skipped_count = (
                            len(full_contacts) - len(contacts_to_update) - len(contacts_to_create)
                        )
                        logger.info(
                            f"migrate_filtered: updated {len(contacts_to_update)} existing contacts, skipped {skipped_count} without changes"
                        )
            except Exception as e:
                ...
                logger.info(
                    f"migrate_filtered: ===== ИМПОРТ КОНТАКТОВ ЗАВЕРШЕН: created={res.contacts_created}, seen={res.contacts_seen}, processed={contacts_processed}, skipped={contacts_skipped}, errors={contacts_errors} ====="
                )
        else:
            logger.info(
                f"migrate_filtered: обработка контактов пропущена: import_contacts={import_contacts}, dry_run={dry_run}, amo_ids={bool(amo_ids)}"
            )
    ...
    logger.info(
        f"  Компаний: seen={res.companies_seen}, matched={res.companies_matched}, batch={res.companies_batch}, created={res.companies_created}, updated={res.companies_updated}"
    )
    logger.info(
        f"  Задач: seen={res.tasks_seen}, created={res.tasks_created}, updated={res.tasks_updated}"
    )
    logger.info(
        f"  Заметок: seen={res.notes_seen}, created={res.notes_created}, updated={res.notes_updated}"
    )
    logger.info(f"  Контактов: seen={res.contacts_seen}, created={res.contacts_created}")
    logger.info(
        f"  CompanyPhone: skynet_added={res.skynet_phones_added}, skynet_rejected={res.skynet_phone_values_rejected}, rejected_invalid={res.company_phones_rejected_invalid}"
    )
