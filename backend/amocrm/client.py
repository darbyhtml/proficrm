...
class AmoClient:
    ...
    def ensure_token(self) -> None:
        ...
        if not self.cfg.access_token:
            raise AmoApiError(
                "Access token не настроен. Проверьте настройки AmoCRM в админке или переавторизуйтесь."
            )
    ...
    def exchange_code(self, code: str) -> None:
        ...
        logger.info(
            f"Exchanging OAuth code. URL: {url}, Client ID: {self.cfg.client_id[:10]}..., Redirect URI: {self.cfg.redirect_uri}"
        )
        ...
        *,
        ...
        try:
            ...
            return AmoResponse(
                ...
                data=_json_loads(raw) or raw.decode("utf-8", errors="replace"),
                headers={},
            )
    ...
    def get(
        self, path: str, *, params: dict[str, Any] | None = None, retry_on_429: bool = True
    ) -> Any:
        """
        ...
        for attempt in range(max_retries):
            ...
            if res.status == 403:
                ...
                if not has_token:
                    ...
                    error_msg += (
                        "  - Токен недействителен или истек (попробуйте переавторизоваться)\n"
                    )
                    error_msg += "  - IP адрес заблокирован\n"
                    error_msg += "  - Неправильный домен AmoCRM\n"
                    error_msg += f"  - Домен: {self.cfg.domain}\n"
                    error_msg += (
                        f"  - Используется long_lived_token: {bool(self.cfg.long_lived_token)}"
                    )
            ...
            if res.status == 429 and retry_on_429:
                if attempt >= max_retries - 1:
                    ...
                    error_msg = (
                        f"GET {path} failed (429): Rate limit exceeded after {max_retries} attempts"
                    )
                ...
                delay = None
                retry_after_header = res.headers.get("retry-after") or res.headers.get(
                    "Retry-After"
                )
        ...
        *,
        ...
        while True:
            ...
            try:
                ...
                logger.debug(
                    f"get_all_pages: endpoint={path}, page={page}, получено элементов на странице: {len(data.get('_embedded', {}).get(embedded_key or 'items', [])) if isinstance(data, dict) else 0}"
                )
            ...
            if embedded_key:
                items = (
                    embedded.get(embedded_key)
                    if isinstance(embedded.get(embedded_key), list)
                    else None
                )
            else:
            ...
            if not items:
                logger.debug(
                    f"get_all_pages: нет элементов на странице {page} для {path}, завершаем пагинацию"
                )
                break
            out.extend(items)
            ...
            if early_stop_callback and early_stop_callback(out):
                logger.info(
                    f"get_all_pages: раннее прерывание пагинации для {path} на странице {page} (получено элементов: {len(out)})"
                )
                break

            if len(items) < limit:
                logger.debug(
                    f"get_all_pages: получено меньше limit ({len(items)} < {limit}) для {path}, завершаем пагинацию"
                )
                break
        ...
        logger.info(
            f"get_all_pages: завершена пагинация для {path}, всего страниц: {pages_fetched}, элементов: {len(out)}"
        )
