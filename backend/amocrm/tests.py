...
class TestNormalizePhone(unittest.TestCase):
            ...
            "custom_fields_values": [
                {
                    ...
                    "values": [{"value": "+7 495 123-45-67", "enum_code": "WORK"}],
                }
            ],
        }
    ...
    def test_extract_phone_from_text(self):
        ...
        assert phone == "+74956322197"
        assert (
            "Ольга" in cleaned or len(cleaned) < 3
        )  # Имя должно быть удалено или остаться минимально
...
class SkynetParsePhonesTests(unittest.TestCase):
    ...
    def test_mixed_valid_and_garbage(self):
        ...
        phones, rejected, examples = parse_skynet_phones(
            "+7 919 111-11-11; мусор; 8 (495) 123-45-67"
        )
        values = [p["value"] for p in phones]
        assert len(values) == 2
    ...
    def test_number_with_instruction_comment(self):
        ...
        phones, rejected, examples = parse_skynet_phones(
            " +7 (912) 384-49-85 временно не доступен "
        )
        assert len(phones) == 1
...
class TestMainPhoneCommentMerge(unittest.TestCase):
    ...
    def test_main_phone_comment_from_regular_phone(self):
        ...
        main_norm = (
            _normalize_phone(self.company.phone) if (self.company.phone or "").strip() else ""
        )
