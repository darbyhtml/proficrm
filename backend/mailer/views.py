...
def mail_admin(request: HttpRequest) -> HttpResponse:
    ...
    """
    enforce(user=request.user, resource_type="page", resource="ui:mail:admin", context={"path": request.path})
    user: User = request.user
    is_admin = require_admin(user)
    if not is_admin:
        messages.error(request, "Доступ запрещён.")
    ...
    if active_tab == "overview":
        ...
        max_per_hour = quota.max_per_hour or 100 if quota.last_synced_at and not quota.sync_error else 100
                ...
                if password:
                    ...
                    if new_api_key and new_api_key != old_api_key:
                        ...
                        try:
                            ...
                            logger.error(f"Ошибка при запуске синхронизации квоты: {e}", exc_info=True)
                    # Проверяем, нужно ли отправить тест
                    if "test_send" in request.POST:
                        to_email = (user.email or "").strip()
                        if not to_email:
                            messages.error(request, "В вашем профиле не задан email — некуда отправить тест.")
                            ...
                            send_test_email.delay(
                                ...
                                from_name=(cfg.from_name or "CRM ПРОФИ").strip(),
                                reply_to=to_email,
                                x_tag="test:mail_admin",
                            )
                            messages.success(request, f"Тестовое письмо поставлено в очередь на отправку. Проверьте ящик {to_email} через несколько секунд.")
                    messages.success(request, "Настройки SMTP сохранены.")
                    return redirect(f"{reverse('mail_admin')}?tab=settings")
            else:
                settings_form = form
        elif request.method == "POST" and "test_send" in request.POST and "update_smtp" not in request.POST:
            enforce(user=request.user, resource_type="action", resource="ui:mail:settings:update", context={"path": request.path, "method": request.method})
            cfg = GlobalMailAccount.load()
            to_email = (user.email or "").strip()
            if not to_email:
        ...
        if request.method == "POST" and "update_limits" in request.POST:
            ...
            try:
                ...
                if limit_val is not None and limit_val > 0:
                    ...
                    messages.success(request, f"Дневной лимит пользователя установлен: {limit_val} писем.")
                else:
                    smtp_cfg.per_user_daily_limit = None
                    smtp_cfg.save(update_fields=["per_user_daily_limit"])
                    messages.success(request, "Дневной лимит пользователя сброшен (без ограничений).")
            except ValueError:
    ...
    if active_tab == "queue":
        ...
        for idx, queue_entry in enumerate(queue_entries_all, 1):
            defer_reason_text = defer_reason_map.get(queue_entry.defer_reason, queue_entry.defer_reason) if queue_entry.defer_reason else ""
            queue_list.append({
                "position": idx,
...
def campaigns(request: HttpRequest) -> HttpResponse:
    ...
    # Пагинация списка кампаний (быстрая отрисовка у всех сотрудников)
    CAMPAIGNS_PER_PAGE = 30
    paginator = Paginator(qs, CAMPAIGNS_PER_PAGE)
    page = paginator.get_page(request.GET.get("page"))
    campaigns_list = list(page.object_list)

    # Аналитика для администратора (кеш агрегатов 60 сек — быстрый отклик страницы)
    analytics = None
    if is_admin:
        from django.utils import timezone as _tz
        now = _tz.now()
    ...
    # Добавляем информацию о паузе и рабочем времени для каждой кампании (campaigns_list уже из пагинации)
    from mailer.tasks import _is_working_hours
    ...
    # Данные для фильтров (филиалы и менеджеры — по всему отфильтрованному списку, не только по странице)
    branches_for_filter = []
    managers_for_filter = []
...
def campaign_start(request: HttpRequest, campaign_id) -> HttpResponse:
    ...
    # Используем функцию, чтобы учитывать override_settings в тестах
