    ...
    for endpoint in ("/user", "/user/stats"):
        for attempt in range(3):
            ...
            if status == 200 and isinstance(data, dict):
                ...
                if (
                    parsed.get("emails_limit", 0) > 0
                    or parsed.get("tariff_name")
                    or parsed.get("emails_available", 0) > 0
                ):
                    logger.debug("smtp.bz API: квота получена с %s", endpoint)
                    return parsed
                logger.debug(
                    "smtp.bz API: %s без полей квоты, ключи: %s", endpoint, list(data.keys())[:10]
                )
                return parsed
...
def _parse_quota_response(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "tarif" in data:
        ...
        result["tariff_name"] = (
            data.get("tariff") or data.get("tariff_name") or data.get("plan", "")
        )
    ...
    if "expires_quota" in data:
        ...
        if renewal_str:
            try:
                result["tariff_renewal_date"] = datetime.strptime(
                    str(renewal_str)[:10], "%Y-%m-%d"
                ).date()
    ...
    if "quota" in data:
        ...
        if isinstance(quota_val, (int, float)):
            ...
            result["emails_available"] = int(
                quota_val.get("available", quota_val.get("remaining", 0))
            )
    ...
    if result["emails_limit"] > 0 and result["emails_available"] == 0:
        ...
        if "sent" in data or "sent_today" in data or "sent_month" in data or "dsent" in data:
            sent = int(
                data.get("sent")
                or data.get("sent_today")
                or data.get("sent_month")
                or data.get("dsent", 0)
            )
    ...
    if "hsent" in data:
        ...
        result["sent_per_hour"] = int(
            data.get("sent_per_hour") or data.get("sentPerHour") or data.get("sent_hour", 0)
        )
    ...
    if "hlimit" in data:
        ...
        "max_per_hour" in data
        or "maxPerHour" in data
        or "hourly_limit" in data
        or "rate_limit" in data
    ):
        result["max_per_hour"] = int(
            data.get("max_per_hour")
            or data.get("maxPerHour")
            or data.get("hourly_limit")
            or data.get("rate_limit", 100)
        )
    ...
    if "quota" in data and isinstance(data["quota"], dict):
        quota = data["quota"]
        result["emails_available"] = int(
            quota.get("available", quota.get("remaining", result["emails_available"]))
        )
    ...
    if "limits" in data and isinstance(data["limits"], dict):
        limits = data["limits"]
        result["sent_per_hour"] = int(
            limits.get("sent_per_hour", limits.get("sent_hour", result["sent_per_hour"]))
        )
        result["max_per_hour"] = int(
            limits.get(
                "max_per_hour",
                limits.get("hourly_limit", limits.get("rate_limit", result["max_per_hour"])),
            )
        )
