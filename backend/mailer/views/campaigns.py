"""
Views для CRUD кампаний: список, создание, редактирование, детали, клонирование, шаблоны.
"""
...
from mailer.forms import CampaignForm, CampaignGenerateRecipientsForm, CampaignRecipientAddForm
...
from mailer.views._helpers import _can_manage_campaign, _contains_links, _smtp_bz_today_stats_cached
...
def campaign_retry_failed(request: HttpRequest, campaign_id) -> HttpResponse:
    ...
    enforce(user=request.user, resource_type="action", resource="ui:mail:campaigns:retry_failed", context={"path": request.path, "method": request.method})
    if request.method != "POST":
        return redirect("campaign_detail", campaign_id=campaign_id)
    camp = get_object_or_404(Campaign, id=campaign_id)
    if not _can_manage_campaign(request.user, camp):
    ...
    if updated:
        ...
        if not created and queue.status not in (CampaignQueue.Status.PENDING, CampaignQueue.Status.PROCESSING):
...
def campaign_export_failed(request: HttpRequest, campaign_id) -> HttpResponse:
    ...
    enforce(user=request.user, resource_type="action", resource="ui:mail:campaigns:export_failed", context={"path": request.path, "method": request.method})
    camp = get_object_or_404(Campaign, id=campaign_id)
    if not _can_manage_campaign(request.user, camp):
    ...
    def rows():
        ...
        for r in camp.recipients.filter(status=CampaignRecipient.Status.FAILED).order_by("email").values_list("email", "last_error", "updated_at"):
