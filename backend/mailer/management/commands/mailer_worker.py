...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--sleep", type=float, default=1.0, help="Пауза между итерациями (сек)."
        )
        parser.add_argument(
            "--batch", type=int, default=50, help="Максимум писем за итерацию на кампанию."
        )
    ...
    def handle(self, *args, **opts):
        ...
        if not bool(opts.get("force")):
            self.stdout.write(
                self.style.ERROR(
                    "mailer_worker устарел. Используйте Celery: mailer.tasks.send_pending_emails (celery + beat)."
                )
            )
            self.stdout.write(
                self.style.ERROR(
                    "Если вы понимаете риск и всё равно хотите запустить: добавьте флаг --force"
                )
            )
            return
        ...
        while True:
            ...
            for camp in camps:
                ...
                batch = list(
                    camp.recipients.filter(status=CampaignRecipient.Status.PENDING)[:allowed]
                )
                if not batch:
                ...
                for r in batch:
                    ...
                    try:
                        ...
                        if hasattr(ex, "original_error"):
                            error_message = str(ex)
                            logger.error(
                                f"Failed to send email {r.email}: {ex.original_error}",
                                exc_info=True,
                            )
                        else:
                        ...
                        if smtp_cfg.smtp_bz_api_key:
                            try:
                                ...
                                message_id = (
                                    str(msg.get("Message-ID", "")).strip().strip("<>").strip()
                                )
                                msg_info = None

                                if message_id:
                                    msg_info = get_message_info(
                                        smtp_cfg.smtp_bz_api_key, message_id
                                    )

                                # Если не нашли по Message-ID, пробуем найти по X-Tag (точнее)
                                if not msg_info:
                                    tag = str(msg.get("X-Tag", "") or "").strip()
                                    start_date = (_tz.now() - _tz.timedelta(minutes=5)).strftime(
                                        "%Y-%m-%d"
                                    )
                                    ...
                                    if tag:
                                        ...
                                        if logs:
                                            messages = (
                                                ...
                                                else logs if isinstance(logs, list) else []
                                            )
                                            if messages:
                                                msg_info = (
                                                    messages[0]
                                ...
                                if not msg_info:
                                    ...
                                    from_email = (
                                        from_email_match.group(0) if from_email_match else ""
                                    )
                                ...
                                if msg_info:
                                    ...
                                    bounce_reason = (
                                        msg_info.get("bounce_reason")
                                        or msg_info.get("error")
                                        or msg_info.get("reason")
                                        or msg_info.get("bounceReason", "")
                                    )
                                ...
                                logger.debug(
                                    f"Failed to get message info from smtp.bz API: {api_error}"
                                )
